...
from metadrive_benchmark.mining.source_bank import SourceBank
from metadrive_benchmark.mining.types import CutInDesign
from metadrive_benchmark.scripts.evaluate_cutin import _require_g1_pass, _source_logical_domain
from metadrive_benchmark.scripts.fit_prior import _select_rank


def _bank() -> SourceBank:
    designs = tuple(
        CutInDesign(candidate, (float(index) / 10.0, ) * 5) for candidate in (0, 1)
...
def test_target_evaluation_domain_is_taken_from_frozen_source_bank() -> None:
    ...
    assert _source_logical_domain([Observation()]) == "mining_source_interaction_v2"
...
def test_target_evaluation_requires_matching_passing_g1_report(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="G1 report is required"):
        _require_g1_pass(str(prior_path), "mining_source_interaction_v2")
    (tmp_path / "source_loso_g1_v2.json").write_text(
        '{"schema":"mining_source_loso_g1_v2","logical_domain_id":"mining_source_interaction_v2","g1_source_only_diagnostic":{"pass":true}}',
        encoding="utf-8",
    )
    _require_g1_pass(str(prior_path), "mining_source_interaction_v2")
