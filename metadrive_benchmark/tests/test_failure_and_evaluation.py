...
from metadrive_benchmark.failure.analyzer import analyze_rollout
from metadrive_benchmark.failure.criteria import FailureCriteria
from metadrive_benchmark.failure.inner_reward import InnerRiskReward
from metadrive_benchmark.failure.metrics import FixedBudgetMetrics
from metadrive_benchmark.failure.signature import FailureSignatureBuilder
...
def test_signature_is_deterministic_and_fixed_budget_counts_unique_failures() -> None:
    ...
    assert first.is_valid_episode and first.is_failure and first == builder.from_outcome(
        outcome, "merge", "zone")
...
def test_analyzer_prefers_a_later_collision_over_an_earlier_near_miss() -> None:
    ...
    outcome, signature = analyze_rollout(transitions, "cutin", "zone", "candidate", criteria)
...
def test_inner_reward_component_interface_preserves_the_scalar_contract() -> None:
    ...
    assert components["reward_preclip"] + components["reward_clip_adjustment"] == pytest.approx(
        total)
    assert components["reward_preclip"] == pytest.approx(components["reward_risk"] +
                                                         components["reward_event"] +
                                                         components["reward_progress"] -
                                                         components["penalty_tracking"] -
                                                         components["penalty_shield"] -
                                                         components["penalty_invalid"])
...
def test_inner_reward_emits_near_miss_bonus_only_on_capture() -> None:
    ...
    assert reward(features, event_info) > reward(features, latched_info)
