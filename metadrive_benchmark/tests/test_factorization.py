...
from metadrive_benchmark.mining.factorization import fit_low_rank_vulnerability
...
def test_global_low_rank_factorization_recovers_rank_one_response() -> None:
    ...
    result = fit_low_rank_vulnerability(values, np.ones_like(values, dtype=bool),
                                        ("a", "b", "c", "d"), tuple(str(i) for i in range(5)), 1)
