...
from metadrive_benchmark.scenario.catalog import mvr_parameter_spaces
...
from metadrive_benchmark.scenario.semantics import ScenarioSemanticMonitor
...
def test_runtime_geometry_hash_and_sut_attachment(family: str) -> None:
    task = next(row for row in load_taskbook("metadrive_benchmark/configs/taskbook.json")
...
def test_runtime_hash_mismatch_fails_fast() -> None:
    task = load_taskbook("metadrive_benchmark/configs/taskbook.json")[0]
