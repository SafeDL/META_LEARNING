...
class MetaDriveFamilyAdapter:
    ...
    @staticmethod
    def adversary_agent_config(layout: ScenarioLayout,
                               config: Mapping[str, float | str]) -> dict[str, Any]:
        return {
    ...
    def _route_from(road_network: Any, lane_index: LaneIndex) -> tuple[tuple[LaneIndex, ...], Any]:
        ...
        current_end = np.asarray(current_lane.position(float(current_lane.length), 0.0),
                                 dtype=float)
    ...
    def resolve_layout(
        ...
        if len(rows) < 2:
            raise RuntimeError(
                "generated map has no two lanes long enough for the selected outer spawn")
        ...
        multi_lane_roads = [
            members
            for _, members in sorted(grouped.items(), key=lambda row: tuple(map(str, row[0])))
            if len(members) >= 2
        ]
        ...
        if shared:
            ...
            conflict = np.asarray(adversary_lane.position(float(adversary_lane.length), 0.0),
                                  dtype=float)
        else:
            conflict = 0.5 * (np.asarray(self._midpoint(adversary_lane)) +
                              np.asarray(self._midpoint(sut_lane)))
