...
def load_geometry_catalog(
        path: str | Path = "metadrive_benchmark/configs/geometry_catalog.json"
) -> dict[str, GeometrySpec]:
    geometries = [
        GeometrySpec.from_dict(row) for row in json.loads(Path(path).read_text(encoding="utf-8"))
    ]
