"""Run one explicit Mining target-SUT protocol after source configuration is frozen."""
...
from ..mining.baselines import TargetOnlyGP
from ..mining.miner import RiskMiner
from ..mining.prior import LowRankVulnerabilityPrior
from ..mining.response import VulnerabilityResponseConfig
from ..mining.source_bank import (
    SourceBank,
...
from ..evaluation.mining_protocol import MiningBudgetLedger
...
from ..mining.validity import RBFEvaluability
...
def _source_logical_domain(observations) -> str:
    ...
    if len(domains) != 1:
        raise ValueError("Mining source bank must contain exactly one frozen logical domain")
    return domains.pop()
...
def _require_g1_pass(prior_path: str, logical_domain_id: str) -> None:
    ...
    if (report.get("schema") != "mining_source_loso_g1_v2"
...
def run(config_path: str,
        ...
        *,
        ...
        support_shots: int,
        algorithm_seed: int,
        method: str = "mining_diagnostic") -> dict:
    config = yaml.safe_load(Path(config_path).read_text(encoding="utf-8"))
    if protocol not in {"adaptation_quality", "frozen_selection", "budget_efficiency"}:
        raise ValueError("unsupported Mining evaluation protocol")
    if method not in METHODS:
        raise ValueError("unsupported Mining evaluation method")
    if sut_split not in {"validation", "test"}:
        raise ValueError("Mining evaluation only accepts a held-out validation or test SUT")
    ...
    bank = SourceBank.from_observations(source_observations,
                                        tuple(config["study"]["source_sut_refs"]))
    features = np.tile(bank.features, (len(bank.source_refs), 1))
    evaluability = RBFEvaluability(features, bank.eligible.reshape(-1),
                                   float(config["validity"]["bandwidth"]))
    ...
    miner = RiskMiner(
        ...
        proxy_event_threshold=float(config["vulnerability_response"]["proxy_event_threshold"]),
    )
    ...
    target_only = TargetOnlyGP(pool, rng, device=str(
        config["device"])) if method == "target_only_gp" else None
    support_count = support_shots if method in {
        "mining_random_support", "mining_diagnostic", "target_only_gp"
    } else 0
    if method in {"random", "frozen_source_mean", "online_mining_k0"
                  } and support_shots not in {0, 4}:
        raise ValueError("this baseline does not use diagnostic support")
    budget = MiningBudgetLedger(20 if protocol == "budget_efficiency" else support_count + 8)
    observations = []
    for index in range(support_count):
        if method == "mining_diagnostic":
            design = miner.select_diagnostic()
        elif method == "mining_random_support":
            design = miner.select_random(rng)
        else:
            assert target_only is not None
            design = target_only.select()
        slot = budget.reserve(phase="support",
                              design_id=design.design_id,
                              episode_seed=algorithm_seed + index)
        observation = executor.run(task,
                                   design,
                                   algorithm_seed + index,
    ...
    remaining = 8 if protocol in {"adaptation_quality", "frozen_selection"
                                  } else budget.total_budget - budget.consumed
    query_designs = (sobol_designs_from_physical_bounds(
        config["study"]["source_physical_bounds"],
        4,
    ...
    for offset in range(remaining):
        ...
        slot = budget.reserve(phase=phase,
                              design_id=design.design_id,
                              episode_seed=algorithm_seed + support_count + offset)
        prediction = None
    ...
    report = {
        "schema": "mining_target_evaluation",
        "protocol": protocol,
        "sut_split": sut_split,
        "sut_ref": target_ref,
        "logical_domain_id": logical_domain_id,
...
def main() -> None:
    ...
    parser.add_argument("--protocol",
                        choices=("adaptation_quality", "frozen_selection", "budget_efficiency"),
                        required=True)
    parser.add_argument("--support-shots", type=int, default=4)
    parser.add_argument("--algorithm-seed", type=int, default=1701)
    parser.add_argument("--method", choices=METHODS, default="mining_diagnostic")
    args = parser.parse_args()
    run(args.config,
        args.source,
        args.prior,
        args.output,
        sut_split=args.sut_split,
