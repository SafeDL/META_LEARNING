...
CUTIN_LOGICAL_DOMAINS = (
    ("cutin_interaction_core", "train", {
        "initial_gap_m": (-0.15, 0.15),
        "ego_initial_speed_mps": (-0.50, 0.50),
        "relative_speed_mps": (-0.60, 0.60),
        "cutin_start_offset_m": (-1.00, 0.00),
        "cutin_path_length_m": (0.00, 0.50),
    }),
    ("cutin_late_fast", "validation", {
        "initial_gap_m": (-0.65, -0.25),
        "ego_initial_speed_mps": (0.55, 0.85),
        "relative_speed_mps": (-0.95, -0.65),
        "cutin_start_offset_m": (0.25, 0.55),
        "cutin_path_length_m": (0.55, 0.85),
    }),
    ("cutin_tight_gap", "test", {
        "initial_gap_m": (0.35, 0.85),
        "ego_initial_speed_mps": (-0.95, -0.55),
        "relative_speed_mps": (0.65, 0.85),
        "cutin_start_offset_m": (0.57, 0.85),
...
def _logical_bounds(family: str, interval: tuple[float, float],
                    mask: tuple[bool, ...]) -> dict[str, tuple[float, float]]:
    return {
...
def _geometry_hash(geometry_id: str) -> str:
    ...
    try:
        ...
        for index in range(len(space.candidates)):
            action = NormalizedScenarioAction(index, (0.0, ) * space.continuous_dim)
...
def build_taskbook(output: str | Path) -> Path:
    ...
    for geometry in geometries:
        ...
        for sut_ref, sut_split in PROFILES:
            ...
            for domain_id, logical_split, interval in domains:
                ...
                bounds = (interval if geometry.functional_scenario == "cutin" else _logical_bounds(
                    geometry.functional_scenario, interval, mask))
                task = ScenarioMiningTaskSpec(
    ...
    hashes_by_split = {
        split: {row["geometry_hash"]
                for row in tasks if row["geometry_split"] == split}
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Build the taskbook from concrete MetaDrive geometries.")
    parser.add_argument("--output", default="metadrive_benchmark/configs/taskbook.json")
