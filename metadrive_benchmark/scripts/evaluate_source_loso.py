"""Offline G1 leave-one-source-out diagnostic evaluation for Mining v2."""
...
from ..mining.acquisition import diagnostic_scores
from ..mining.factorization import fit_low_rank_vulnerability
...
from .analyze_prior import analyze_bank
from .fit_prior import _select_rank
...
def _ndcg(actual: np.ndarray, predicted: np.ndarray, count: int = 8) -> float:
    ...
    return float(np.dot(gains, discount) /
                 np.dot(ideal, np.ones_like(ideal))) if np.sum(ideal) > 0.0 else 0.0
...
def _auc(labels: np.ndarray, probabilities: np.ndarray) -> float | None:
    ...
    return float(
        (ranks[positives].sum() - count * (count + 1) / 2.0) / (count * (len(labels) - count)))


def _metrics(actual: np.ndarray, mean: np.ndarray, variance: np.ndarray,
             posterior: LatentVulnerabilityPosterior, threshold: float) -> dict:
...
def _one_run(bank: SourceBank, held: int, rank: int, method: str, shots: int, seed: int,
             threshold: float, noise_var: float) -> dict:
    train_rows = [index for index in range(len(bank.source_refs)) if index != held]
    factor = fit_low_rank_vulnerability(bank.responses[train_rows], bank.eligible[train_rows],
                                        tuple(bank.source_refs[index] for index in train_rows),
                                        bank.design_ids, rank)
    ...
    for candidate in _candidate_schedule(shots, rng):
        ...
        choices = choices[np.asarray(
            [bank.designs[index].candidate_index == candidate for index in choices])]
    ...
    variance = np.einsum("ij,jk,ik->i", factor.basis[query], posterior.covariance,
                         factor.basis[query]) + noise_var
...
def _aggregate(records: list[dict]) -> dict:
    numeric = ("rmse", "ndcg_at_8", "top8_recall", "brier", "posterior_logdet",
        ...
        for seed in seeds:
            ...
            for shots in (1, 2, 4):
                for method in METHODS[1:]:
                    records.append(
                        _one_run(bank, held, rank, method, shots, seed, threshold, noise))
    aggregates = {
        f"{method}_k{k}":
        _aggregate([row for row in records if row["method"] == method and row["k"] == k])
        for method in METHODS for k in ((0, ) if method == "shared_prior" else (1, 2, 4))
    }
    primary = int(config["gates"]["g1"]["primary_k"])
    diagnostic = [
        row for row in records if row["method"] == "diagnostic_support" and row["k"] == primary
    ]
    random = [row for row in records if row["method"] == "random_support" and row["k"] == primary]
    paired_delta = np.asarray(
        [a["ndcg_at_8"] - b["ndcg_at_8"] for a, b in zip(diagnostic, random)])
    lower, upper = _bootstrap_ci(paired_delta)
    criteria = {
        "ndcg_gain_vs_k0":
        aggregates[f"diagnostic_support_k{primary}"]["ndcg_at_8"] -
        aggregates["shared_prior_k0"]["ndcg_at_8"] >= config["gates"]["g1"]["min_ndcg_gain_vs_k0"],
        "positive_paired_ndcg_ci":
        lower > 0.0
        if config["gates"]["g1"]["require_positive_paired_ndcg_ci_vs_random"] else True,
        "rmse_non_degradation":
        aggregates[f"diagnostic_support_k{primary}"]["rmse"] <=
        (1.0 + config["gates"]["g1"]["max_rmse_relative_degradation_vs_random"]) *
        aggregates[f"random_support_k{primary}"]["rmse"],
    }
    report = {
        "schema": "mining_source_loso_g1_v2",
        "config_hash": content_hash(config),
        "logical_domain_id": config["study"]["logical_domain_id"],
        "selected_rank": rank,
        "rank_selection": selection,
        "aggregates": aggregates,
        "paired_diagnostic_vs_random_k4": {
            "mean_delta_ndcg": float(paired_delta.mean()),
            "ci95": [lower, upper]
        },
        "g1_source_only_diagnostic": {
