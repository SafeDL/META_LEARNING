"""Evaluate the source-only formal teacher before model training."""
...
from ..formal.formal_counterfactual_teacher import (
    BASELINE_POLICIES,
    FormalCounterfactualTeacher,
)
from ..formal.formal_calibrator import formal_class_indexes
...
def _load_bank(path: Path, source_refs: tuple[str, ...]) -> SourceBank:
    ...
    if set(bank.source_refs) & FORBIDDEN_SOURCE_REFS:
        raise ValueError("sealed validation/test SUT appears in the formal-teacher source bank")
    return bank


def _read_inputs(config_path: str | Path) -> tuple[Path, dict, dict, SourceBank]:
    config_file = Path(config_path)
    root = REPOSITORY_ROOT
    config = yaml.safe_load(config_file.read_text(encoding="utf-8"))
    if config.get("schema") != "formal_teacher_formal_teacher":
        raise ValueError("formal-teacher validator requires its matching config")
    if not str(config["results_dir"]).replace("\\", "/").endswith("/formal_teacher"):
        raise ValueError("formal-teacher results require an isolated directory")
    ...
    return root, config, base_config, bank


def build_manifest(config_path: str | Path) -> dict[str, Any]:
    root, config, _, bank = _read_inputs(config_path)
    manifest = {
        "schema":
        "formal_teacher_stage1_manifest_v2",
        "git_commit":
        _git_commit(root),
        "source_refs":
        list(bank.source_refs),
        "source_only":
        True,
        "new_simulator_calls":
        0,
        "sealed_sut_refs":
        sorted(FORBIDDEN_SOURCE_REFS),
        "budgets": {
            "primary": int(config["protocol"]["primary_budget"]),
            "final": int(config["protocol"]["final_budget"]),
        },
        "rebuild_command": ("conda run -n metadrive python -m "
                            "metadrive_benchmark.scripts.validate_formal_teacher "
                            "--config metadrive_benchmark/configs/formal_teacher.yaml"),
    }
...
def _teachers(bank: SourceBank, config: dict,
              base_config: dict) -> dict[int, FormalCounterfactualTeacher]:
    ...
    for held in range(len(bank.source_refs)):
        ...
        result[held] = FormalCounterfactualTeacher.from_source_bank(
            bank,
            training,
...
def _replay_rows(
    bank: SourceBank,
    teachers: dict[int, FormalCounterfactualTeacher],
    budgets: tuple[int, ...],
) -> list[dict[str, Any]]:
    final_budget = max(budgets)
    policies = ("teacher", *BASELINE_POLICIES)
        ...
        for source_index in training:
            for policy in policies:
                ...
                row = {
                    ...
                    held,
                    "held_out_source":
                    bank.source_refs[held],
                    "source_sut":
                    bank.source_refs[source_index],
                    "policy":
                    policy,
                    "selected_design_ids":
                    [bank.design_ids[index] for index in evaluation.selected_indexes],
                    "cumulative_vulnerability_at_20":
                    float(evaluation.vulnerability_curve[-1]),
                }
...
def _best_baseline(rows: list[dict[str, Any]], metric: str,
                   sources: set[str]) -> tuple[str, float]:
    values = {policy: _mean(rows, policy, metric, sources) for policy in BASELINE_POLICIES}
...
def _headroom_audit(
    ...
    for row in rows:
        replay_groups.setdefault((int(row["fold"]), row["source_sut"]), {})[row["policy"]] = row
    ...
    for budget in budgets:
        ...
        score_policy, score_baseline = _best_baseline(rows, f"score_at_{budget}", event_sources)
    ...
    criteria = {
        ...
        (aggregate["10"]["terminal_headroom"] >= float(threshold["min_score_headroom_at_10"])),
        "auc_headroom_at_10":
        aggregate["10"]["auc_headroom"] > 0.0,
        "auc_headroom_at_20":
        aggregate["20"]["auc_headroom"] > 0.0,
        "event_source_auc_headroom":
        sum(value["headroom"] > 0.0 for value in source_auc_headroom.values()) >= int(
            threshold["min_event_sources_with_auc_headroom"]),
    }
    return {
        "schema": "formal_teacher_stage1_headroom_audit_v2",
...
def _formal_metrics(
    teacher: FormalCounterfactualTeacher,
    bank: SourceBank,
    held: int,
    method: str,
    shots: int,
    ...
    valid_probabilities = bool(
        np.isfinite(probabilities).all() and np.all(probabilities >= 0.0)
...
def _formal_alignment_gate(
    ...
    for held, teacher in teachers.items():
        for seed in seeds:
            records.append(
                _formal_metrics(teacher, bank, held, "diagnostic_support", 0, seed, ndcg_k))
            records.append(
                _formal_metrics(teacher, bank, held, "diagnostic_support", shots, seed, ndcg_k))
            records.append(
                _formal_metrics(teacher, bank, held, "random_support", shots, seed, ndcg_k))

    def selected(method: str, k: int, *, event_only: bool) -> list[dict[str, Any]]:
        return [
            row for row in records if row["method"] == method and row["k"] == k and (
                not event_only or row["held_out_source"] in event_sources)
        ]
    ...
    diagnostic_ndcg = float(np.mean([row["formal_ndcg_at_10"] for row in diagnostic_event]))
    paired = np.asarray(
        [
    ...
    diagnostic_brier = float(np.mean([row["multiclass_brier"] for row in diagnostic_all]))
    thresholds = config["gates"]["g1_c"]
    criteria = {
        "formal_ndcg_gain_vs_k0":
        (diagnostic_ndcg - k0_ndcg >= float(thresholds["min_formal_ndcg_gain_vs_k0"])),
        "positive_paired_ndcg_ci_vs_random":
        lower > 0.0,
        "brier_non_degradation":
        diagnostic_brier <=
        (1.0 + float(thresholds["max_brier_relative_degradation_vs_k0"])) * k0_brier,
        "valid_probabilities":
        all(row["valid_probabilities"] for row in records),
        "held_out_not_in_calibrator_fit":
        all(held not in teachers[held].calibrator.training_source_indexes for held in teachers),
    }
    return {
        "schema": "formal_teacher_stage1_formal_alignment_gate_v2",
        "aggregates": {
        ...
        "calibrators": {
            str(held): {
                "held_out_source":
                bank.source_refs[held],
                "training_source_indexes":
                list(teacher.calibrator.training_source_indexes),
                "class_counts":
                list(teacher.calibrator.class_counts),
            }
            for held, teacher in teachers.items()
        },
        "criteria": criteria,
...
def _diagnosis_gate(root: Path, config: dict) -> dict[str, Any]:
    reference = json.loads((root / config["source_loso_reference"]).read_text(encoding="utf-8"))
    ...
    criteria = {
        "ndcg_gain_vs_k0":
        diagnostic["ndcg_at_8"] - k0["ndcg_at_8"] >= float(thresholds["min_ndcg_gain_vs_k0"]),
        "positive_paired_ndcg_ci":
        float(ci[0]) > 0.0,
        "rmse_non_degradation":
        diagnostic["rmse"] <=
        (1.0 + float(thresholds["max_rmse_relative_degradation_vs_random"])) * random["rmse"],
    }
    return {
        "schema": "formal_teacher_stage1_diagnosis_gate_v2",
...
def _signal_rows(
    ...
    for held, teacher in teachers.items():
        ...
        for total_budget in (10, 20):
            contexts = tuple(
                int(value) for value in config["teacher"][f"signal_context_sizes_b{total_budget}"])
            for source_index in training:
                state = teacher.initial_state()
                for context_size in contexts:
                    ...
                    formal = np.asarray([value.normalized_q_auc_formal for value in values])
                    ...
                    result.append({
                        ...
                        remaining,
                        "formal_p90_p10":
                        float(np.percentile(formal, 90) - np.percentile(formal, 10)),
                        "vulnerability_p90_p10":
                        float(np.percentile(vulnerability, 90) - np.percentile(vulnerability, 10)),
                    })
    return result
...
def _teacher_gate(
    ...
    first_teacher = [
        row["first_critical_step"] or 21 for row in rows
    ...
    best_first_policy, best_first = min(first_baselines.items(),
    ...
    for fold in sorted({int(row["fold"]) for row in rows}):
        teacher_values = [
            row["auc_at_20"] for row in rows if row["fold"] == fold and row["policy"] == "teacher"
            and row["source_sut"] in event_sources
        ]
        baseline_values = [
            row["auc_at_20"] for row in rows if row["fold"] == fold
            and row["policy"] == auc20_baseline_policy and row["source_sut"] in event_sources
        ]
    ...
    formal_signal = float(
        np.mean([
            row["formal_p90_p10"] >= float(thresholds["min_formal_normalized_p90_p10"])
            for row in signal_rows
        ])) if signal_rows else 0.0
    vulnerability_signal = float(
        np.mean([
            row["vulnerability_p90_p10"] >= float(
                thresholds["min_vulnerability_normalized_p90_p10"]) for row in signal_rows
        ])) if signal_rows else 0.0
    score10 = aggregates["score_at_10"]
    relative_threshold = float(thresholds["min_relative_gain"])
    criteria = {
        "score_at_10_noninferior":
        score10["absolute_gain"] >= 0.0,
        "score_at_10_gain":
        score10["absolute_gain"] >= float(thresholds["min_score_absolute_gain_at_10"]) or
        (score10["relative_gain"] is not None and score10["relative_gain"] >= relative_threshold),
        "auc_at_10_gain":
        aggregates["auc_at_10"]["relative_gain"] is not None
        and aggregates["auc_at_10"]["relative_gain"] >= relative_threshold,
        "auc_at_20_gain":
        aggregates["auc_at_20"]["relative_gain"] is not None
        and aggregates["auc_at_20"]["relative_gain"] >= relative_threshold,
        "score_at_20_noninferior":
        aggregates["score_at_20"]["absolute_gain"] >= 0.0,
        "first_critical_noninferior":
        teacher_first <= best_first,
        "positive_fold_consistency":
        sum(value > 0.0
            for value in fold_gains.values()) >= int(thresholds["min_positive_fold_count"]),
        "nondegenerate_formal_q":
        formal_signal >= float(thresholds["min_formal_signal_rate"]),
        "nondegenerate_vulnerability_q":
        vulnerability_signal >= float(thresholds["min_vulnerability_signal_rate"]),
        "fixed_budget_and_no_duplicates":
        all(
            len(row["selected_design_ids"]) == 20 and len(set(row["selected_design_ids"])) == 20
            for row in rows),
    }
    return {
        "schema": "formal_teacher_stage1_teacher_gate_v2",
...
def run(config_path: str | Path) -> dict[str, Any]:
    ...
    calibrator_path.write_text(json.dumps(calibrator_artifact, indent=2) + "\n", encoding="utf-8")
    budgets = tuple(int(value) for value in config["protocol"]["reporting_budgets"])
    ...
    (results_dir / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n",
                                               encoding="utf-8")
    unlocked = all(report["pass"] for report in (audit, diagnosis, formal_alignment, teacher))
    gate = {
        "schema": "formal_teacher_stage1_gate_v2",
        "source_only": True,
        "new_simulator_calls": 0,
        "g1_a_headroom": {
            "pass": audit["pass"],
            "report": "headroom_audit.json"
        },
        "g1_b_diagnosis": {
            "pass": diagnosis["pass"],
            "report": "diagnosis_gate.json"
        },
        ...
        "transformer": {
            "trained": False,
            "status": "unlocked_not_implemented" if unlocked else "prohibited_by_formal_gate",
        },
        "decision":
        "ready_for_transformer_implementation" if unlocked else "stop_before_transformer",
        "failure_reasons": {
    ...
    (results_dir / "stage1_gate.json").write_text(json.dumps(gate, indent=2) + "\n",
                                                  encoding="utf-8")
    return gate
...
def main() -> None:
    ...
    parser.add_argument("--config", default="metadrive_benchmark/configs/formal_teacher.yaml")
