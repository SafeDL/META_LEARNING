...
class FailureSignatureBuilder:
    ...
    def from_outcome(
        ...
        raw_invalid = any(
            bool(outcome.get(key, False)) for key in (
        ...
        near_miss = bool(
            outcome.get("valid_critical_near_miss", outcome.get("valid_critical_strict", False)))
        is_failure = is_valid_episode and (collision or near_miss)
        if collision:
            failure_type = "target_collision"
        elif near_miss:
            failure_type = "valid_critical_near_miss"
        else:
            failure_type = "none"
        severity = (
            self._bin(
