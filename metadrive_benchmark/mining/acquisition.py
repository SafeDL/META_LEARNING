"""Pure Mining diagnostic and failure-mining acquisition functions."""
...
def score_level_set_weight(mean: np.ndarray, variance: np.ndarray, threshold: float) -> np.ndarray:
...
def diagnostic_scores(
    ...
    *,
    ...
    boundary = score_level_set_weight(predicted_mean, predicted_variance, level_set_threshold)
    return information * (eta +
                          (1.0 - eta) * boundary) * np.power(evaluability, evaluability_power)
...
def novelty_weight(features: np.ndarray,
                   archive: np.ndarray,
                   bandwidth: float = 0.50) -> np.ndarray:
