...
class LowRankVulnerabilityPrior:
    ...
    def fit(cls,
            ...
            *,
            device: str = "cuda",
            gp_fit_steps: int = 100) -> "LowRankVulnerabilityPrior":
        factorization = fit_low_rank_vulnerability(bank.responses, bank.eligible, bank.source_refs,
        ...
        for candidate in (0, 1):
            ...
            if len(indexes) < 2:
                raise ValueError(
                    "each candidate requires two common eligible anchors for GP fitting")
            gp_bank.fit(
                candidate,
    ...
    def predict(
        self, designs: tuple[CutInDesign, ...], posterior: LatentVulnerabilityPosterior
        ...
        for candidate in (0, 1):
            indexes = np.asarray(
                [i for i, design in enumerate(designs) if design.candidate_index == candidate])
            if not len(indexes):
                continue
            ...
            if self.rank:
                interpolation = interpolation + (
                    (np.square(posterior.mean) + np.diag(posterior.covariance)) *
                    basis_var).sum(axis=1)
            means[indexes] = mean + basis @ posterior.mean
            noise[
                indexes] = self.noise_floor_var + self.factorization.residual_variance + interpolation
    ...
    def load(cls, path: str | Path, *, map_location: str = "cpu") -> "LowRankVulnerabilityPrior":
        ...
        if not isinstance(prior, cls):
            raise ValueError("artifact is not a Mining low-rank vulnerability prior")
        return prior
