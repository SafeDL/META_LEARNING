"""Aligned Mining source banks and source-domain Sobol sampling."""
...
from .types import MINING_SCHEMA, CutInDesign, MiningObservation
...
class SourceBank:
    ...
    designs: tuple[CutInDesign, ...]
    formal_scores: np.ndarray
    responses: np.ndarray
    eligible: np.ndarray
    observations: tuple[MiningObservation, ...]
    ...
    def from_observations(
        cls, observations: Iterable[MiningObservation], source_refs: tuple[str, ...]
    ) -> "SourceBank":
        ...
        grouped: dict[tuple[int, int], list[MiningObservation]] = {}
        for row in rows:
...
def retained_task(
    ...
    if len(matches) != 1:
        raise ValueError("retained Mining task selector must resolve exactly one task")
    return matches[0]
...
def study_task(
    ...
    """Create a Mining-only task view for frozen physical study bounds."""
...
def sobol_designs_from_physical_bounds(
    ...
    """Sample the frozen Mining study domain, then encode globally for execution."""
    ...
    for candidate in (0, 1):
        ...
        while len(accepted) < anchors_per_candidate:
            ...
            for values in values_batch:
                ...
                accepted.append(
                    CutInDesign(candidate, tuple(map(float, action.continuous)))
                )
...
def sobol_designs(
    ...
    if task.functional_scenario != "cutin":
        raise ValueError("Mining Sobol casebook requires Cut-in tasks")
    names = logical_parameter_names("cutin")
    space = mvr_parameter_spaces()["cutin"]
...
def observation_from_dict(payload: Mapping[str, Any]) -> MiningObservation:
    if payload.get("schema") != MINING_SCHEMA:
        raise ValueError("incompatible observation schema; v1 sparse labels are not accepted")
    design_payload = dict(payload["design"])
    if design_payload.pop("schema", None) != MINING_SCHEMA:
        raise ValueError("incompatible Mining design schema")
    design_payload.pop("design_id", None)
    row = dict(payload)
    row.pop("schema", None)
    row["design"] = CutInDesign(**design_payload)
    return MiningObservation(**row)
