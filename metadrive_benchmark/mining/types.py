"""Immutable Mining contracts independent of the simulator and SUT identity."""
...
MINING_SCHEMA = "mining_mine_cutin_constant_speed_physical_v2"
...
class CutInDesign:
    ...
    def validate(self) -> None:
        ...
        if self.candidate_index not in (0, 1):
            raise ValueError("Mining Cut-in candidate_index must be 0 or 1")
        if not np.isfinite(values).all() or any(abs(float(value)) > 1.0 for value in values):
            raise ValueError("Mining design controls must be finite normalized values")
    ...
    def design_id(self) -> str:
        return content_hash({"schema": MINING_SCHEMA, **asdict(self)})

    def to_dict(self) -> dict[str, Any]:
        return {"schema": MINING_SCHEMA, "design_id": self.design_id, **asdict(self)}


@dataclass(frozen=True)
class MiningObservation:
    ...
    def __post_init__(self) -> None:
        if self.status not in {"valid_event", "completed_noncritical", "censored", "invalid"}:
            raise ValueError("unknown Mining observation status")
        if float(self.score) not in (0.0, 0.5, 1.0):
            raise ValueError("Mining score must use the formal valid-critical scale")
        ...
        if self.posterior_eligible:
            if self.vulnerability_response is None or not np.isfinite(self.vulnerability_response):
                raise ValueError(
                    "eligible Mining observation requires a finite vulnerability response")
            ...
            raise ValueError("ineligible Mining observation must not carry a vulnerability response")

    def to_dict(self) -> dict[str, Any]:
        payload = asdict(self)
        payload["schema"] = MINING_SCHEMA
