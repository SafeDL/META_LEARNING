...
本文档说明仓库中已经实现的 PEARL-SAC 流程、冻结实验协议与当前保留的正式结果；它不再是待实现的需求清单。默认 dense 配置为 [`dense_pearl_baseline.yaml`](../../pearl_learning/configs/dense_pearl_baseline.yaml)，最终结果的机器可读来源为 [`../results/pearl_learning/key_results.json`](../../results/pearl_learning/key_results.json)。

后续 posterior-routed MoE 研发不属于本文所述“已实现基线”；其研究阶段、硬停止条件和交付契约见 [Posterior-Routed MoE 研发路线](POSTERIOR_ROUTED_MOE_ROADMAP.md)。
...
正式 PEARL 训练必须显式传入 `--formal-run` 和与 taskbook 哈希相匹配的 formal validation。该验证要求 topology/integrity audit 通过、任务异质性审计表明 pooled SAC 弱于 per-task SAC、存在正的匹配环境步数预算，并已完成 `per_task_sac`、`cross_task_policy_matrix`、`topology_conditioned_pooled_sac`、`scratch_sac`、`pooled_finetune_sac` 与 `oracle_task_conditioned_sac` 基线。
