...
本文以当前工程代码、冻结的最终实验材料和已通过 smoke 验证的候选实现为依据编写。除非明确标为“已完成”或给出冻结结果数值，否则内容均为待检验的研究目标，不能视为实验结论。
...
第一阶段先建立不改变当前主流程的诊断。已完成部分为：从 taskbook 和允许的 support case 参数导出不含任务 ID、哈希、query 数据或默认隐藏规则的几何、交互先验和可见规则描述，并输出最近邻覆盖报告；同时仅从 support 轨迹导出现有 PEARL 后验及其方差收缩；并完成验证任务上的逐任务预算匹配收益采集和“证据不足即拒绝阈值”的校准接口。为解除仅有 2 个验证任务的限制，工程已提供独立候选 taskbook 构建器：在不改动选定实验的前提下，将验证集扩展至 8 个几何任务，并已通过真实地图解析、完整性审计和 24/24 拓扑审计。候选输入上的首个 5,000 步、种子 20260728 异质性筛选得到 per-task 减 topology-conditioned pooled 的平均严格有效关键率差为 0.010，低于预注册门槛 0.020，故正式 gate 按设计拒绝；该失败结果不会通过降低门槛处理。随后以文档示例的种子 0、20,000 步进行了独立诊断，表面差值为 0.605；复核发现此运行中的 pooled SAC 仅获得跨 10 个训练任务合计 20,000 步，而 per-task SAC 是每个任务各 20,000 步，且 pooled 严格有效关键率在 10 个训练任务上均为 0。该预算不对称使该差值不能作为异质性通过证据，故结果仅保留为实现诊断，不得进入正式 gate。工程现已把预算写入每个 baseline manifest：正式异质性审计要求 per-task、跨任务矩阵与 pooled 的每任务步数相同，并要求 pooled 总步数等于每任务步数乘训练任务数；不满足即拒绝审计。随后在候选 taskbook 的 10 个训练任务上完成种子 1、2 的公平比较：每个 per-task SAC 各训练 20,000 步，topology-conditioned pooled SAC 对同一 10 个任务总计训练 200,000 步，跨任务矩阵仅执行评估。两种子审计的 per-task 减 pooled 平均 `valid_critical_strict_rate` 分别为 0.220 和 0.100，跨种子均值为 0.160，高于预注册门槛 0.020，故审计以 `status=pass` 通过；taskbook 哈希在全部输入中一致。任务级差异存在明显方差，部分任务 pooled 更优，因此该结果只证明候选任务集合中存在可重复的策略专用化收益，不能推断所有任务均需专门训练，也不能作为 PEARL、表示解耦或 OOD 泛化有效性的证据。该通过结果仅解除进入正式 PEARL 与预算匹配基线比较的前置限制。成功标准仍是完成可重复的相关性与校准分析，而不是宣称已实现 OOD 泛化。
