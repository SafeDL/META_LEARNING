...
class InteractionCandidate:
    ...
    def features(self) -> np.ndarray:
        ...
        values = np.asarray((
            math.sin(self.crossing_angle_rad),
            math.cos(self.crossing_angle_rad),
    ...
    def from_layout(cls, env: Any, layout: ScenarioLayout) -> "InteractionCandidate":
        adversary = RoutePolyline.from_env(env, {
            "route_id": "adversary",
    ...
    def from_routes(
        ...
        def curvature(route: RoutePolyline, s_m: float) -> float:
            index = int(
                np.clip(np.searchsorted(route.arc_lengths_m, s_m), 0,
                        len(route.points) - 2))
            ...
            return float(
                np.arctan2(first[0] * second[1] - first[1] * second[0], np.dot(first, second)))
        ...
        return cls(
            ...
            adversary_s,
            curvature(sut, sut_s),
            curvature(adversary, adversary_s),
