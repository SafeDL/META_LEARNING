...
class ScenarioMiningTaskSpec:
    ...
    def validate(self) -> None:
        ...
        if not all(
                str(value) for value in (
                    ...
                    self.geometry_id,
                    self.adapter_id,
                    self.interaction_schema_id,
                    self.logical_domain_id,
                )):
    ...
    def active_logical_parameter_names(self) -> tuple[str, ...]:
        return tuple(name for name, active in zip(
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "ScenarioMiningTaskSpec":
        ...
        if unknown or missing:
            raise ValueError(
                f"task fields mismatch; unknown={sorted(unknown)}, missing={sorted(missing)}")
        payload = dict(value)
