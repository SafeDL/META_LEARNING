...
class ScenarioActionAdapter:
    ...
    def _frenet_contract(self) -> FrenetManeuverContract:
        ...
        if self.family == "cutin":
            ...
            target_indices = tuple((start, end, traffic.target_lane_number)
                                   for start, end, _ in episode.layout.adversary_route)
            route = RoutePolyline.from_env(
                episode.env,
                {
                    "route_id": "maneuver-spine",
                    "lane_sequence": target_indices
                },
            )
            start_s = float(parameters["cutin_start_s_m"])
            start_lateral = route.projection(episode.adversary.position,
                                             episode.adversary.heading_theta).lateral_m
    ...
    def apply_planner_action(
        ...
        *,
        ...
        active = bool(self.state.maneuver_latched and not self.state.maneuver_completed
    ...
    def target_lane(self) -> Any:
        ...
        number = (contract.target_lane_number if self.state.maneuver_latched
                  and contract.target_lane_number is not None else contract.source_lane_number)
        current = self.episode.adversary.navigation.current_lane.index
        return self.episode.env.current_map.road_network.get_lane((current[0], current[1], number))
    ...
    def observe_semantics(self, challenge_phase_active: bool, maneuver_completed: bool) -> None:
...
class ScenarioSemanticMonitor:
    ...
    def _shared_conflict_active(self) -> bool:
        adv_s = self.episode.adversary_route.projection(self.episode.adversary.position,
                                                        self.episode.adversary.heading_theta).s_m
        sut_s = self.episode.sut_route.projection(self.episode.sut.position,
                                                  self.episode.sut.heading_theta).s_m
        ...
        radius = (self.merge_conflict_radius_m
                  if self.family == "merge" else self.conflict_radius_m)
    ...
    def _vehicle_overlaps_lane_corridor(vehicle: Any, lane: Any) -> bool:
        ...
        lane_heading = float(
            lane.heading_theta_at(float(np.clip(longitudinal, 0.0, float(lane.length)))))
        ...
        footprint_longitudinal = (abs(np.cos(heading_delta)) * half_length +
                                  abs(np.sin(heading_delta)) * half_width)
        footprint_lateral = (abs(np.sin(heading_delta)) * half_length +
    ...
    def update(self) -> SemanticState:
        if self.family == "cutin":
            ...
            adversary_lateral = abs(
                float(target.local_coordinates(self.episode.adversary.position)[1]))
            sut_lateral = abs(float(target.local_coordinates(self.episode.sut.position)[1]))
            intrusion = self._vehicle_overlaps_lane_corridor(self.episode.adversary, target)
            ...
            state = SemanticState(started, active, self._completed, challenge, intrusion, active)
        else:
            if self.family == "merge":
                ...
                challenge = self._shared_conflict_active()
            state = SemanticState(challenge, challenge, challenge, challenge, False, challenge)
        self._state = state
        self.schedule.observe_semantics(state.challenge_phase_active, state.maneuver_completed)
        return state

    @staticmethod
    def _execution_valid(info: Mapping[str, Any]) -> bool:
        return not any(
            bool(info.get(key, False)) for key in (
    ...
    def capture_event(self, kind: str | None, info: Mapping[str, Any]) -> bool:
        ...
        if self._event_kind == "collision" or (self._event_kind == "near_miss"
                                               and kind == "near_miss"):
            return False
