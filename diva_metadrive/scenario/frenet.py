...
class FrenetManeuverContract:
    ...
    def __post_init__(self) -> None:
        values = np.asarray((
            self.start_s_m,
            self.start_lateral_m,
            self.end_lateral_m,
            self.min_length_m,
            self.max_length_m,
            self.corridor_lower_m,
            self.corridor_upper_m,
...
class FrenetPath:
    ...
    def evaluate(self, s_m: float) -> tuple[float, float, float, float]:
        ...
        first = np.asarray(
            [0.0 if index == 0 else index * progress**(index - 1) for index in range(8)],
            dtype=float)
        second = np.asarray([
            0.0 if index < 2 else index * (index - 1) * progress**(index - 2) for index in range(8)
        ],
                            dtype=float)
        lateral = float(np.dot(self.coefficients, powers))
        slope = float(np.dot(self.coefficients, first) / self.length_m)
        second_derivative = float(np.dot(self.coefficients, second) / self.length_m**2)
        ...
        *,
        ...
        local_limits = np.asarray([
            min(
                ...
                float(
                    np.sqrt(REFERENCE_LATERAL_ACCELERATION_MPS2 /
        ...
        for index in range(len(positions) - 2, -1, -1):
            ...
            reachable = np.sqrt(allowed[index + 1]**2 +
...
def _build_path(
    ...
    base_progress = np.asarray([float(quintic_smoothstep(value)) for value in ANCHOR_PROGRESS])
    progress = np.clip(
        base_progress + SHAPE_PROGRESS_SCALE * np.asarray((beta_early, beta_late), dtype=float),
        0.0,
        1.0,
    )
    ...
    anchors = contract.start_lateral_m + (contract.end_lateral_m -
                                          contract.start_lateral_m) * progress
    targets = np.asarray((
        contract.start_lateral_m,
        0.0,
        0.0,
        contract.end_lateral_m,
        0.0,
        0.0,
        anchors[0],
        anchors[1],
...
def decode_frenet_path(
    ...
    length = contract.min_length_m + 0.5 * (action[0] + 1.0) * (contract.max_length_m -
                                                                contract.min_length_m)
...
class FrenetPathPlanner:
    ...
    def blend_progress(self) -> float:
        return float(np.clip(self._blend_step / self.replan_interval_steps, 0.0, 1.0))
    ...
    def apply(
        ...
        if maneuver_active and not self._locked and self.replan_due:
            ...
            requested_length = self.contract.min_length_m + 0.5 * (float(action[0]) + 1.0) * (
                self.contract.max_length_m - self.contract.min_length_m)
            feasible_length = float(
                np.clip(requested_length, minimum_length, self.contract.max_length_m))
            ...
            requested[1:] = (requested_path.beta_early, requested_path.beta_late)
            ...
            if not feasible:
                ...
                if _speed_feasible(
                    ...
                    for _ in range(30):
                        ...
                        candidate = decode_frenet_path(self.contract, candidate_action)
                        if _speed_feasible(
                                candidate,
                                current_s_m,
                                current_speed_mps,
                    ...
                    selected = decode_frenet_path(self.contract,
                                                  neutral + lower * (requested - neutral))
                    feasible = True
                else:
            ...
            effective_lambda = 0.0 if interval <= 1e-6 else (
                2.0 * (selected.length_m - self.contract.min_length_m) / interval - 1.0)
            ...
            self._blend_step = 0
        effective = np.concatenate((self._active_action, action[3:4])).astype(np.float32)
        if maneuver_active:
            self._maneuver_steps += 1
            self._blend_step = min(self._blend_step + 1, self.replan_interval_steps)
        return effective
    ...
    def reference_points(self, samples: int = 81) -> list[list[float]]:
        ...
        points = []
        for s_m in np.linspace(self.contract.start_s_m, path.end_s_m, num=samples, dtype=float):
            _, lateral, _, _ = path.evaluate(float(s_m))
            points.append(self.contract.spine.position_at_s(float(s_m), lateral).tolist())
        return points

    def _blended_path(self) -> FrenetPath:
        weight = float(quintic_smoothstep(self.blend_progress))
        coefficients = ((1.0 - weight) * self._previous_path.coefficients +
                        weight * self._target_path.coefficients)
        length = ((1.0 - weight) * self._previous_path.length_m +
                  weight * self._target_path.length_m)
        return FrenetPath(
            self.contract,
            float(length),
            float((1.0 - weight) * self._previous_path.beta_early +
                  weight * self._target_path.beta_early),
            float((1.0 - weight) * self._previous_path.beta_late +
                  weight * self._target_path.beta_late),
            coefficients,
        )
    ...
    def reference(self, position: Any, heading: float) -> FrenetReferenceState:
        ...
        progress, desired_lateral, slope, lateral_second = path.evaluate(projection.s_m)
