...
class TrafficBehaviorContract:
    ...
    def __post_init__(self) -> None:
        ...
        if min(self.speed_limit_mps,
               self.sut_nominal_speed_mps) <= 0.0 or not self.allowed_lane_numbers:
...
class NativeNavigationContract:
    ...
    def expected_sut_lane_number(self, road: tuple[Any, Any]) -> int:
        for index, lane_number in zip(zip(self.sut_checkpoints[:-1], self.sut_checkpoints[1:]),
                                      self.sut_lane_sequence):
