...
class MergeScenarioAdapter(MetaDriveFamilyAdapter):
            ...
            for downstream, lanes in road_network.graph.get(junction, {}).items():
                for lane_number, common_lane in enumerate(lanes):
                    ...
                    if len(by_source) < 2:
                        continue
                    source_widths = {
                        source: sum(1 for index, _ in distinct if index[0] == source)
                        for source in by_source
                    }
                    ramp_source = min(source_widths,
                                      key=lambda source: (source_widths[source], str(source)))
                    main_source = max(source_widths,
                                      key=lambda source: (source_widths[source], str(source)))
    ...
    def resolve_layout(
        ...
        downstream_route, adversary_destination = self._route_from(road_network, common)
    ...
    def env_config(self,
                   task: ScenarioMiningTaskSpec,
                   config: Mapping[str, float | str],
                   layout: ScenarioLayout | None = None) -> dict[str, Any]:
        result = {
