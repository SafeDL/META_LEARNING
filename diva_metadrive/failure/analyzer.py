...
def analyze_rollout(
    ...
    outcome = {
        ...
        any(bool(info.get("non_target_collision", False)) for info in infos),
        "adversary_out_of_road":
        any(bool(info.get("adversary_out_of_road", False)) for info in infos),
        "sut_out_of_road":
        any(bool(info.get("sut_out_of_road", False)) for info in infos),
        "wrong_route":
        any(bool(info.get("wrong_route", False)) for info in infos),
        "target_collision":
        any(
            bool(info.get("target_collision", info.get("crash_vehicle", False)))
            for info in infos),
        "min_ttc":
        min(float(row[8]) * 15.0 for row in features),
        "min_distance":
        min(float(row[10]) * 100.0 for row in features),
        "max_closing_speed":
        max(
            float(row[10]) * 100.0 / max(float(row[8]) * 15.0, 1e-3) if row[8] < 1.0 else 0.0
            for row in features),
    }
    challenge_features = [
        feature for feature, info in zip(features, infos)
        if bool(info.get("semantic_challenge_phase_active", False))
    ]
    outcome["challenge_min_ttc"] = (min(
        float(row[8]) * 15.0 for row in challenge_features) if challenge_features else None)
    outcome["challenge_min_distance"] = (min(
        float(row[10]) * 100.0 for row in challenge_features) if challenge_features else None)
    final_info = infos[-1]
    ...
    collision_events = [info for info in event_infos if info.get("event_kind") == "collision"]
    ...
    outcome["event_execution_valid"] = bool(event_info.get("event_execution_valid", False))
    ...
    if traffic_infos:
        ...
        outcome["control_telemetry"] = {
            "requested_actions":
            [list(info["traffic_requested_action"]) for info in traffic_infos],
            ...
            "rejection_counts":
            dict(final_traffic["traffic_rejection_counts"]),
            "max_speed_mps":
            float(final_traffic["traffic_max_speed_mps"]),
            "max_abs_acceleration_mps2":
            float(final_traffic["traffic_max_abs_acceleration_mps2"]),
            "max_abs_jerk_mps3":
            float(final_traffic["traffic_max_abs_jerk_mps3"]),
            "max_lateral_acceleration_mps2":
            float(final_traffic["traffic_max_lateral_acceleration_mps2"]),
            "legal_lane_lateral_m":
            float(final_traffic["traffic_legal_lane_lateral_m"]),
            "raw_longitudinal":
            [float(info["control_raw_longitudinal"]) for info in traffic_infos],
            "speed_limited_longitudinal":
            [float(info["control_speed_limited_longitudinal"]) for info in traffic_infos],
            "projected_longitudinal":
            [float(info["control_projected_longitudinal"]) for info in traffic_infos],
            "curvature_speed_limit_mps":
            [float(info["control_curvature_speed_limit_mps"]) for info in traffic_infos],
            "distance_envelope_speed_limit_mps":
            [float(info["control_distance_envelope_speed_limit_mps"]) for info in traffic_infos],
            "path_projection_scale":
            [float(info["control_path_projection_scale"]) for info in traffic_infos],
            "path_speed_feasible":
            [bool(info["control_path_speed_feasible"]) for info in traffic_infos],
        }
    onset_infos = [
        info for info in infos if bool(info.get("cutin_actual_onset_just_started", False))
    ]
    if onset_infos:
        onset = onset_infos[0]
        outcome["cutin_actual_onset"] = {
            ...
            "adversary_speed_mps": float(onset["cutin_actual_onset_adversary_speed_mps"]),
    ...
    if sut_infos:
        outcome["sut_telemetry"] = {
            ...
            "routing_target_lanes": [
                None if info["sut_routing_target_lane"] is None else list(
                    info["sut_routing_target_lane"]) for info in sut_infos
            ],
            "current_ref_lanes": [[list(lane) for lane in info["sut_current_ref_lanes"]]
            ...
            "curve_safe_speed_mps":
            [float(info["sut_curve_safe_speed_mps"]) for info in sut_infos],
        }
    invalid = any(
        bool(outcome[key]) for key in (
            "non_target_collision",
            "adversary_out_of_road",
            "sut_out_of_road",
            "wrong_route",
        ))
    event_valid = bool(outcome["event_semantic_valid"] and outcome["event_execution_valid"])
    outcome["valid_target_collision"] = bool(outcome["target_collision"]
                                             and outcome["event_kind"] == "collision"
                                             and event_valid)
    outcome["valid_critical_near_miss"] = (outcome["event_kind"] == "near_miss" and event_valid)
    ...
    invalid = invalid and not (outcome["valid_target_collision"]
                               or outcome["valid_critical_near_miss"])
    outcome["is_valid_episode"] = not invalid
    signature = FailureSignatureBuilder(criteria).from_outcome(outcome, scenario_family,
                                                               conflict_zone_id, candidate_id)
    outcome.update({
        "is_valid_episode": signature.is_valid_episode,
        "is_failure": signature.is_failure,
        "is_collision": bool(outcome["valid_target_collision"]),
        "is_near_miss": bool(outcome["valid_critical_near_miss"]),
        "severity_vector": signature.severity_vector,
