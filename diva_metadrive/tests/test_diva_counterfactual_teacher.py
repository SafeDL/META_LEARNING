...
from diva_metadrive.diva_ai.counterfactual_teacher import CONTINUATION_POLICIES, CounterfactualTeacher
from diva_metadrive.diva_ai.state import TeacherWorld


def _teacher() -> CounterfactualTeacher:
    pool = tuple(DivaCutInDesign(index % 2, (index / 10.0, ) * 5) for index in range(6))
    world = TeacherWorld(
        pool=pool,
