...
from diva_metadrive.diva.response import VulnerabilityResponseConfig, compute_vulnerability_response
from diva_metadrive.diva.types import DivaCutInDesign, DivaObservation
...
def test_observation_requires_response_exactly_when_eligible() -> None:
    ...
    with pytest.raises(ValueError):
        DivaObservation(status="completed_noncritical",
                        posterior_eligible=True,
                        vulnerability_response=None,
                        **common)
    with pytest.raises(ValueError):
        DivaObservation(status="censored",
