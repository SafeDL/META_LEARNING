"""Strict accounting contracts for DIVA target-SUT evaluation."""
...
class DivaBudgetLedger:
    ...
    def reserve(self, *, phase: str, design_id: str, episode_seed: int) -> int:
        if phase not in {"support", "mining", "query"}:
            raise ValueError("unknown DIVA evaluation phase")
        if len(self.entries) >= self.total_budget:
            raise RuntimeError("DIVA target episode budget is exhausted")
        entry = {
