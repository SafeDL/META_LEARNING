...
class TrafficActionShield:
    ...
    def _steering_limit(self) -> float:
        ...
        maximum = np.arctan(self.max_lateral_acceleration_mps2 * wheelbase / max(speed, 0.25)**2)
    ...
    def project(
        ...
        if self.schedule.family != "cutin":
            ...
            jerk_reference = (self._previous_acceleration_mps2 if self._previous_acceleration_mps2
                              is not None else self._previous_command_acceleration_mps2)
            ...
            if command_acceleration < 0.0:
                stop_safe_deceleration = np.sqrt(2.0 * self.max_jerk_mps3 * max(speed, 0.0))
            ...
            action[1] = self._acceleration_action(command_acceleration, available_acceleration,
                                                  available_deceleration)
        ...
        rate_limited = float(
            np.clip(action[0], current_steering - steering_delta,
                    current_steering + steering_delta))
        ...
        if self.schedule.family == "cutin" and self.schedule.state.maneuver_latched:
            ...
            if not np.isclose(requested[0], 0.0) and (
                    predicted_lateral > safe_upper - anticipation_margin or lateral > safe_upper):
                ...
                reasons.append("lateral_corridor")
                    ...
                    predicted_lateral < safe_lower + anticipation_margin or lateral < safe_lower):
                action[0] = float(-steering_limit)
                reasons.append("lateral_corridor")
    ...
    def _legal_lane_lateral(self) -> float:
        vehicle = self.episode.adversary
        projection = self.episode.adversary_route.projection(vehicle.position,
    ...
    def observe(self, shielded: ShieldedAction) -> dict[str, Any]:
        ...
        speed = self._speed_mps(vehicle)
        dt = self._step_seconds(self.episode.env)
        acceleration = (0.0 if self._previous_speed_mps is None else
                        (speed - self._previous_speed_mps) / dt)
        ...
        if previous_filtered_acceleration is None:
            ...
            filtered_acceleration = 0.5 * (previous_filtered_acceleration + acceleration)
        ...
        if self.schedule.family == "cutin":
            ...
            if self._previous_cutin_lateral_m is not None:
                self._cutin_lateral_velocity_mps = (
                    current_cutin_lateral - self._previous_cutin_lateral_m) / max(dt, 1e-6)
            self._previous_cutin_lateral_m = current_cutin_lateral
        max_steering_rad = np.deg2rad(float(vehicle.config.get("max_steering", 40.0)))
        wheelbase = max(0.6 * self._vehicle_length(vehicle), 1.0)
        lateral_acceleration = speed**2 * np.tan(
            float(shielded.action[0]) * max_steering_rad) / wheelbase
        ...
        self._max_lateral_acceleration_mps2 = max(self._max_lateral_acceleration_mps2,
                                                  abs(lateral_acceleration))
        lateral = self._legal_lane_lateral()
        route_projection = self.episode.adversary_route.projection(vehicle.position,
                                                                   vehicle.heading_theta)
        ...
        return {
            "traffic_shield_rejected":
            shielded.rejection_reason is not None,
            "traffic_shield_rejection_reason":
            shielded.rejection_reason,
            "traffic_requested_action":
            shielded.requested_action.tolist(),
            "traffic_executed_action":
            shielded.action.tolist(),
            "traffic_shield_intervention_l2":
            intervention,
            "traffic_rejection_counts":
            dict(self._rejections),
            "traffic_max_speed_mps":
            self._max_speed_mps,
            "traffic_max_abs_acceleration_mps2":
            self._max_abs_acceleration_mps2,
            "traffic_max_abs_jerk_mps3":
            self._max_abs_jerk_mps3,
            "traffic_max_lateral_acceleration_mps2":
            self._max_lateral_acceleration_mps2,
            "traffic_acceleration_mps2":
            acceleration,
            "traffic_jerk_mps3":
            jerk,
            "traffic_legal_lane_lateral_m":
            lateral,
            "traffic_route_transition_active":
            self.episode.adversary_route.in_lane_change(route_projection.s_m),
            ...
            "traffic_cutin_lateral_corridor_m":
            cutin_corridor,
        }
