...
from ..diva_ai.formal_counterfactual_teacher import (
    BASELINE_POLICIES,
...
def _read_inputs(config_path: str | Path) -> tuple[Path, dict, dict, SourceBank]:
    ...
    if config.get("schema") != "diva_former_formal_teacher":
...
def build_manifest(config_path: str | Path) -> dict[str, Any]:
    ...
    manifest = {
        "schema":
        "diva_former_stage1_manifest_v2",
        "git_commit":
        _git_commit(root),
        "source_refs":
        ...
        "rebuild_command": ("conda run -n metadrive python -m "
                            "diva_metadrive.scripts.validate_diva_former "
                            "--config diva_metadrive/configs/diva_former.yaml"),
    }
...
def main() -> None:
    ...
    parser.add_argument("--config", default="diva_metadrive/configs/diva_former.yaml")
