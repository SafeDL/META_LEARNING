...
class MapPolyline:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.polyline_id or points.ndim != 2 or points.shape[1] != 2 or len(
                points) < 2 or not np.isfinite(points).all():
            raise ValueError("polyline requires at least two finite XY points")
        if np.asarray(self.headings).shape != (len(points), ) or np.asarray(
                self.curvature).shape != (len(points), ):
    ...
    def local_features(self) -> np.ndarray:
        ...
        return np.column_stack(
            (local, np.cos(self.headings - heading), np.sin(self.headings - heading),
             self.curvature, np.full(len(points), self.lane_width),
             np.full(len(points), self.speed_limit))).astype(np.float32)
...
class MapTokens:
    ...
    def tensorize(
        ...
        features = torch.as_tensor(np.stack(
            [polyline.local_features() for polyline in self.polylines]),
                                   device=device)
        centres = torch.as_tensor(np.stack(
            [np.asarray(polyline.points_xy).mean(axis=0) for polyline in self.polylines]),
                                  device=device)
        headings = torch.as_tensor(
            [polyline.headings[len(polyline.headings) // 2] for polyline in self.polylines],
            device=device)
