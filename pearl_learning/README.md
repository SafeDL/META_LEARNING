...
```powershell
conda run -n metadrive python -m pearl_learning.scripts.build_taskbook --config pearl_learning/configs/posterior_adaptation_protocol.yaml --output results/pearl_learning/posterior_adaptation/taskbooks
```

该命令同时在 taskbook 的父目录生成 `casebooks/`。

训练入口为 `pearl_learning/scripts/train_pearl.py`。正式训练必须完成配置规定的多个训练种子和 validation freeze；smoke 只能验证流程，不能替代正式训练。

配对 ID/OOD 评估示例：

```powershell
conda run -n metadrive python -m pearl_learning.scripts.evaluate_fewshot --config pearl_learning/configs/posterior_adaptation_protocol.yaml --checkpoint <checkpoint> --taskbook results/pearl_learning/posterior_adaptation/taskbooks --casebook-root results/pearl_learning/posterior_adaptation --split meta_test_all --run-name <run-name> --validation-freeze-manifest <freeze.json>
```
