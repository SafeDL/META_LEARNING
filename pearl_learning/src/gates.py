...
def verify_formal_gate(path: str | Path | None, taskbook_hash: str) -> None:
    ...
    if payload.get("topology_audit") != "pass" or payload.get("integrity_audit") != "pass":
        raise RuntimeError("formal gate lacks a passing all-task topology/integrity audit")
    if payload.get("heterogeneity_audit") != "pass":
        raise RuntimeError("formal gate lacks evidence that pooled SAC underperforms per-task SAC")
    if int(payload.get("baseline_environment_steps", 0)) <= 0:
        raise RuntimeError("formal gate lacks one matched positive environment-step budget for all SAC baselines")
