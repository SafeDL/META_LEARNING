...
def main() -> None:
    ...
    parser.add_argument("--smoke", action="store_true"); parser.add_argument("--gate-manifest"); parser.add_argument("--output-root"); parser.add_argument("--resume-checkpoint"); parser.add_argument("--checkpoint-interval-steps", type=int); parser.add_argument("--no-topology", action="store_true"); parser.add_argument("--topology-dropout", type=float, default=0.0); parser.add_argument("--topology-dropout-warmup-steps", type=int, default=0)
    ...
    result = train(cfg, tasks, validation, casebooks, content_hash(taskbook_payload(taskbook)), max_steps, args.seed, args.run_name, args.smoke, args.gate_manifest, args.resume_checkpoint, args.checkpoint_interval_steps)
