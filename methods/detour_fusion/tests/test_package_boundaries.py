...
from methods.detour_fusion.experiment import OUTPUT_DIR
...
def test_base_and_standalone_replication_do_not_own_fusion_modules():
    forbidden = (
        REPOSITORY / "highway_sim_env" / "config.py",
        REPOSITORY / "highway_sim_env" / "mining" / "detour_transfer.py",
        REPOSITORY / "highway_sim_env" / "experiments" / "run_multifunction_detour_fusion.py",
        REPOSITORY / "highway_sim_env" / "scripts" / "render_multifunction_gifs.py",
...
def test_dependency_direction_is_one_way_into_the_fusion_chain():
    base = REPOSITORY / "highway_sim_env"
    reverse_imports = [
        path
        for path in base.rglob("*.py")
        if "methods.detour_fusion" in path.read_text(encoding="utf-8")
    ]
    assert not reverse_imports

    fusion_source = (CHAIN / "fusion.py").read_text(encoding="utf-8")
    assert "highway_sim_env" in fusion_source
