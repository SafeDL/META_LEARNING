...
```powershell
conda run -n metadrive python -B -m methods.function_posterior_search.confirmation
conda run -n metadrive python -B -m pytest methods/function_posterior_search/tests -q -p no:cacheprovider
```
