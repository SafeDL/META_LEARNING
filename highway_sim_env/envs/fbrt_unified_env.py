...
class FBRTUnifiedEnv(AbstractEnv):
    def __init__(self, spec: BuildSpec, scenario: dict):
        ...
        self.observed_maneuver_phase: str | None = None
    ...
    def _reset(self) -> None:
        ...
        self.observed_maneuver_phase = None
    ...
    def _collision_phase(self) -> str:
        """Classify the current geometry, including an event on this physics step."""
        ego = self.vehicle
        if ego.target_lane_index != self.initial_ego_lane_index:
            ...
            if (ego.lane_index == ego.target_lane_index and
                ...
                return "after_ego_lane_change"
            return "during_ego_lane_change"
        if "ego_lane_change_complete_s" in self.event_times:
            return "after_ego_lane_change"
        lead = self.actors.get("lead")
        if getattr(lead, "merge_completed_at", None) is not None:
            return "after_lead_merge"
        return "current_lane_or_pre_merge"
        ...
        for role, actor in self.actors.items():
            ...
            if lateral_overlap:
                ...
                if ego.crashed and self.collision_partner_role is None and clearance < 0.25:
                    ...
                    self.observed_maneuver_phase = self._collision_phase()

        # Highway-env's collision shapes and Shapely polygons can differ at the
        # contact boundary. A simultaneously crashed actor is direct evidence.
        if ego.crashed and self.collision_partner_role is None:
            crashed_actors = [(role, actor) for role, actor in self.actors.items()
                              if role != "ego" and actor.crashed]
            if crashed_actors:
                self.collision_partner_role = min(
                    crashed_actors, key=lambda pair: ego_polygon.distance(
                        Polygon(pair[1].polygon())))[0]
                self.collision_time_s = stamp
                self.observed_maneuver_phase = self._collision_phase()
    ...
    def result(self) -> dict:
        ...
        collision_type = None
        if ego_collision:
            if self.collision_partner_role == "rear":
                collision_type = "lane_change_side_rear" if self.observed_maneuver_phase == \
                    "during_ego_lane_change" else "rear_collision"
            elif self.collision_partner_role in {"lead", "adjacent_front"}:
                collision_type = ("post_lane_change_front" if self.observed_maneuver_phase ==
                                  "after_ego_lane_change" else
                                  "lane_change_front" if self.observed_maneuver_phase ==
                                  "during_ego_lane_change" else "current_lane_front")
            else:
                collision_type = "unknown_ego_collision"
        return {
            ...
            "ego_runtime": {
                ...
                "rear_guard_rejected_count": int(getattr(self.vehicle, "rear_guard_rejected_count", 0)),
                "aged_rear_prediction_count": int(getattr(self.vehicle, "aged_rear_prediction_count", 0)),
                "lane_change_accepted_count": int(getattr(self.vehicle, "lane_change_accepted_count", 0)),
