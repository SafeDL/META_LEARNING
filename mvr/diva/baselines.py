"""Small, explicit DIVA comparators with the same no-repeat and budget contracts."""
...
from .acquisition import novelty_weight
...
class TargetOnlyGP:
    ...
    def select(self) -> DivaCutInDesign:
        ...
        by_candidate = {candidate: [row for row in usable if row.design.candidate_index == candidate] for candidate in (0, 1)}
        ...
        for candidate in (0, 1):
            ...
            query_indexes = [index for index, design in enumerate(available) if design.candidate_index == candidate]
        ...
        ties = [available[index] for index, value in enumerate(scores) if np.isclose(value, maximum)]
