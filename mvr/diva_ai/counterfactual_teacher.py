...
class CounterfactualTeacher:
    ...
    def from_source_bank(
        ...
        *,
        ...
        noise = np.full(
            len(bank.designs), 0.02 + factorization.residual_variance, dtype=np.float64
        )
        world = TeacherWorld(
            pool=bank.designs,
    ...
    def action_value(
        ...
        if remaining_budget == 1:
            return TeacherActionValue(
                candidate_index, immediate_formal, immediate_vulnerability,
                "terminal", immediate_formal, immediate_vulnerability,
            )
        ...
        for policy in CONTINUATION_POLICIES:
            ...
            future_formal, future_vulnerability = self._continuation(
                branch, source_index, remaining_budget - 1, policy
            )
            returns.append(
                (policy, immediate_formal + future_formal, immediate_vulnerability + future_vulnerability)
            )
    ...
    def run(self, source_index: int, total_budget: int, policy: str) -> TeacherEvaluation:
        ...
        for step in range(total_budget):
            ...
            if policy == "teacher":
                ...
                best = max(values, key=lambda value: (value.q_formal, value.q_vulnerability, -value.candidate_index))
