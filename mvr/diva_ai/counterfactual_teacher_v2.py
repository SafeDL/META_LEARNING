"""Time-sensitive Stage 1-v2 counterfactual utility teacher."""
...
class TeacherEvaluationV2:
    ...
    def first_critical_step(self) -> int | None:
        return next((index + 1 for index, score in enumerate(self.formal_curve) if score > 0.0), None)


class CounterfactualTeacherV2:
    ...
    def _observe_oracle(self, state: TeacherState, source_index: int, candidate_index: int) -> None:
        self.base._observe_oracle(state, source_index, candidate_index)

    def formal_mining_values(self, state: TeacherState) -> np.ndarray:
        archive = (
            self.world.features[np.asarray(state.failure_indexes, dtype=int)]
            if state.failure_indexes
            else np.empty((0, self.world.features.shape[1]), dtype=np.float64)
        )
        return (
            self.calibrator.expected_utility(state)
            * np.square(self.world.evaluability)
    ...
    def _select_index(self, state: TeacherState, policy: str) -> int:
        ...
        if policy != "formal_mining":
            raise ValueError("unknown Stage 1-v2 selection policy")
    ...
    def _continuation_schedule(policy: str) -> tuple[int, str]:
        ...
        if policy not in schedules:
            raise ValueError("unknown Stage 1-v2 continuation policy")
        return schedules[policy]

    def _continuation(
        self, state: TeacherState, source_index: int, remaining_steps: int, policy: str
    ...
    def action_value(
        self, state: TeacherState, source_index: int, candidate_index: int, remaining_budget: int
        ...
        if remaining_budget == 1:
            return TeacherActionValueV2(
                candidate_index, formal, vulnerability, formal, vulnerability,
                formal, vulnerability, "terminal", "terminal", formal, vulnerability,
                tuple(float(value) for value in probabilities), expected,
            )

        returns: list[tuple[str, float, float, float, float]] = []
        for policy in V2_CONTINUATION_POLICIES:
            branch = state.clone()
    ...
    def action_values(
        self, state: TeacherState, source_index: int, remaining_budget: int
    ) -> list[TeacherActionValueV2]:
        return [
    ...
    def run(
        self, source_index: int, total_budget: int, policy: str
    ) -> TeacherEvaluationV2:
        ...
        for step in range(total_budget):
            ...
            if policy == "teacher":
                ...
                if step < 4:
                    ...
                    mining = "formal_mining" if policy.endswith("formal") else "vulnerability_mining"
                ...
                raise ValueError("unknown Stage 1-v2 replay policy")
            formal = float(self.world.formal_scores[source_index, candidate])
            vulnerability = float(self.world.responses[source_index, candidate])
            self._observe_oracle(state, source_index, candidate)
            formal_total += formal
