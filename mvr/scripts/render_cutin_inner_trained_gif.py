...
def run(config_path: str, checkpoint_path: str, output_dir: str) -> dict[str, Any]:
    ...
    if len(tasks) != 1:
        raise ValueError("Cut-in trained GIF rendering requires exactly one training task")
    task = tasks[0]
    ...
    for episode_index in (episodes_per_task // 3, 2 * episodes_per_task // 3):
        label = f"trained SAC deterministic episode {episode_index:02d}"
        frames, callback = _capture_frames(label, action_key="planner_action")
        result = online.run(
            task,
            1,
            deterministic=True,
        ...
        path = output / f"trained_episode_{episode_index:02d}.gif"
