"""Create the frozen v2 DIVA source casebook from study physical bounds."""
...
from ..diva.source_bank import retained_task, sobol_designs_from_physical_bounds
from ..diva.types import DIVA_SCHEMA
...
CASEBOOK_SCHEMA = "diva_cutin_source_casebook_v2"
    ...
    for design in designs:
        ...
        for name in names:
            ...
            if not lower <= float(values[name]) <= upper:
                raise ValueError("DIVA design escaped frozen source physical bounds")
        if not valid_cutin_initial_state(
            ...
            raise ValueError("DIVA design violates Cut-in physical reset constraints")
...
def run(config_path: str, output_path: str) -> dict:
    ...
    payload = {
        ...
        "observation_schema": DIVA_SCHEMA,
