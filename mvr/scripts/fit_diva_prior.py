...
def _support_indexes(bank: SourceBank, indexes: np.ndarray, shots: int, seed: int) -> np.ndarray:
    ...
    per_candidate = {candidate: indexes[np.asarray([bank.designs[i].candidate_index == candidate for i in indexes])] for candidate in (0, 1)}
...
def _select_rank(
    ...
    *,
    ...
    for held in range(len(bank.source_refs)):
        ...
        for rank in candidates:
            factor = fit_low_rank_vulnerability(
                train, train_eligible, refs, bank.design_ids, rank
            )
            for seed in seeds:
    ...
    metrics = {
        str(rank): {
            "nll": float(np.mean(losses[rank])) if losses[rank] else float("inf"),
            "rmse": float(np.sqrt(np.mean(squared_errors[rank]))) if squared_errors[rank] else float("inf"),
        }
        for rank in candidates
    }
...
def run(config_path: str, source_path: str, output_path: str, rank: int | None = None) -> dict:
    ...
    prior = LowRankVulnerabilityPrior.fit(
        bank, chosen, device=str(config["device"]), gp_fit_steps=int(config["prior"]["gp_fit_steps"])
    )
    ...
    manifest = {
        ...
        "rank": chosen,
        "rank_selection": {**{f"rank_{key}": value for key, value in selection.items()}, "selected_rank": selected},
