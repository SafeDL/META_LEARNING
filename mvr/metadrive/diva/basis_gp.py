...
class _MaternExactGP(gpytorch.models.ExactGP):
    def __init__(self, features: torch.Tensor, targets: torch.Tensor, likelihood: gpytorch.likelihoods.GaussianLikelihood) -> None:
...
class _ExactMaternGP:
    ...
    @classmethod
    def fit(cls, features: np.ndarray, targets: np.ndarray, *, device: str, steps: int = 100) -> "_ExactMaternGP":
        ...
        likelihood = gpytorch.likelihoods.GaussianLikelihood().to(target_device, dtype=torch.float64)
...
class BasisGPBank:
    ...
    def fit(self, candidate_index: int, features: np.ndarray, mean_target: np.ndarray, basis_targets: np.ndarray) -> None:
        ...
        mean_model = _ExactMaternGP.fit(
            features, mean_target, device=self.device, steps=self.fit_steps
        )
        basis_models = tuple(
            _ExactMaternGP.fit(
    ...
    def predict(self, candidate_index: int, query: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        try:
        ...
        return mean, mean_var, np.column_stack([row[0] for row in predicted]), np.column_stack([row[1] for row in predicted])
