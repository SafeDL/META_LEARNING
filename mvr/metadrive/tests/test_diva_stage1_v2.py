...
from mvr.metadrive.diva_ai.counterfactual_teacher import CounterfactualTeacher
from mvr.metadrive.diva_ai.counterfactual_teacher_v2 import (
    V2_CONTINUATION_POLICIES,
    CounterfactualTeacherV2,
    TeacherEvaluationV2,
)
from mvr.metadrive.diva_ai.formal_calibrator import FormalEventCalibrator
from mvr.metadrive.diva_ai.state import TeacherWorld
from mvr.metadrive.scripts.validate_diva_former_stage1_v2 import _jaccard
...
def _world(count: int = 24) -> TeacherWorld:
    pool = tuple(
        DivaCutInDesign(index % 2, ((index / max(count - 1, 1)) * 2.0 - 1.0,) * 5)
...
def test_auc_teacher_prefers_earlier_reward_when_terminal_return_ties() -> None:
    ...
    teacher = CounterfactualTeacherV2(CounterfactualTeacher(world), _MeanCalibrator())
...
def test_v2_terminal_identity_and_portfolio_values() -> None:
    world = _world(6)
    teacher = CounterfactualTeacherV2(CounterfactualTeacher(world), _MeanCalibrator())
    state = teacher.initial_state()
...
def test_v2_replay_obeys_both_exact_budgets(budget: int) -> None:
    world = _world(24)
    teacher = CounterfactualTeacherV2(CounterfactualTeacher(world), _MeanCalibrator())
...
def test_v2_curve_metrics_and_overlap_are_exact() -> None:
    evaluation = TeacherEvaluationV2(
        policy="toy",
        source_index=0,
        total_budget=4,
