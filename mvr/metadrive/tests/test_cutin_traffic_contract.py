...
from mvr.metadrive.control import FrenetSACAdversaryController
from mvr.metadrive.physical_limits import CUTIN_LATERAL_ACCELERATION_LIMIT_MPS2
from mvr.metadrive.scenario.catalog import (
    CUTIN_REFERENCE_LATERAL_ACCELERATION_MPS2,
    mvr_parameter_spaces,
)
from mvr.metadrive.scenario.executor import ScenarioExecutor
from mvr.metadrive.scenario.parameter_space import NormalizedScenarioAction
from mvr.metadrive.scenario.registry import load_adapters
from mvr.metadrive.scenario.semantics import (
...
from mvr.metadrive.scenario.frenet import REFERENCE_LATERAL_ACCELERATION_MPS2
from mvr.metadrive.state import INNER_STATE_FIELDS, PhysicalStateExtractor
from mvr.metadrive.scenario.taskbook import load_taskbook
from mvr.metadrive.safety.dynamics import (
    CUTIN_MAX_LATERAL_ACCELERATION_MPS2,
    CUTIN_VEHICLE_CONFIG,
)
from mvr.metadrive.safety import TrafficActionShield
from mvr.metadrive.training.runner import HierarchicalRunner


def _episode(candidate_index: int = 0, *, environment_overrides=None):
    task = next(
        task
        for task in load_taskbook("mvr/metadrive/configs/taskbook.json")
