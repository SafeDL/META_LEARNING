...
def test_executor_rejects_action_outside_task_logical_domain_before_simulation() -> None:
    task = next(task for task in load_taskbook("mvr/metadrive/configs/taskbook.json") if task.logical_split == "validation")
...
def test_executor_rejects_inactive_maneuver_onset(family: str) -> None:
    task = next(
        task for task in load_taskbook("mvr/metadrive/configs/taskbook.json")
