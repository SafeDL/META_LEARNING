...
from mvr.metadrive.diva_ai.counterfactual_teacher import CONTINUATION_POLICIES, CounterfactualTeacher
from mvr.metadrive.diva_ai.state import TeacherState, TeacherWorld


def _teacher() -> CounterfactualTeacher:
    pool = tuple(
        DivaCutInDesign(index % 2, (index / 10.0,) * 5)
...
def test_teacher_portfolio_is_not_worse_than_each_continuation_policy() -> None:
    ...
    assert value.q_formal >= teacher.world.formal_scores[0, candidate] + max(row[0] for row in returns)
    assert value.q_vulnerability >= teacher.world.responses[0, candidate] + max(row[1] for row in returns)
