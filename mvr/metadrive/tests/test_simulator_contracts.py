...
from mvr.metadrive.scenario.catalog import mvr_parameter_spaces
...
from mvr.metadrive.scenario.semantics import ScenarioSemanticMonitor
...
from mvr.metadrive.sut.idm import LaneStableNativeIDMPolicy
...
def test_runtime_geometry_hash_and_sut_attachment(family: str) -> None:
    task = next(
        row for row in load_taskbook("mvr/metadrive/configs/taskbook.json")
...
def test_runtime_hash_mismatch_fails_fast() -> None:
    task = load_taskbook("mvr/metadrive/configs/taskbook.json")[0]
