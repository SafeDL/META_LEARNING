...
from mvr.metadrive.scripts.evaluate_diva_cutin import _require_g1_pass, _source_logical_domain
from mvr.metadrive.scripts.fit_diva_prior import _select_rank
...
def _bank() -> SourceBank:
    ...
    return SourceBank(("a", "b", "c", "d"), designs, formal, responses, np.ones_like(responses, dtype=bool), ())
...
def test_rank_selection_reports_deterministic_loso_metrics() -> None:
    ...
    reordered = SourceBank(bank.source_refs, tuple(reversed(bank.designs)), bank.formal_scores[:, ::-1], bank.responses[:, ::-1], bank.eligible[:, ::-1], ())
...
def test_target_evaluation_domain_is_taken_from_frozen_source_bank() -> None:
    ...
    with pytest.raises(ValueError, match="exactly one"):
        _source_logical_domain([Observation(), type("Other", (), {"logical_domain_id": "other"})()])
