...
def test_interaction_prior_visits_one_task_per_episode_in_balanced_epochs(monkeypatch) -> None:
    ...
    class FakeOnline:
        def run(self, task, budget, **kwargs):
            calls.append((task.task_id, budget, kwargs))
    ...
    assert [kwargs["episode_index_offset"] for _, _, kwargs in calls] == [0, 0, 0, 1, 1, 1]
    assert all(kwargs["inner_gamma"] == 0.97 for _, _, kwargs in calls)
