...
def test_distance_to_conflict_is_preserved_on_distinct_geometries(geometry_id: str) -> None:
    ...
    episode = ScenarioExecutor(load_adapters(), mvr_parameter_spaces()).reset(task, action)
    try:
        assert episode.applied_scenario.adversary_distance_to_conflict_m == pytest.approx(2.75)
        assert episode.applied_scenario.sut_distance_to_conflict_m == pytest.approx(2.75)
