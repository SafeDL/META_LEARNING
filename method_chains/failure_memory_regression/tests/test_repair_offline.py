...
from method_chains.failure_memory_regression.repair_replay import _summaries
from method_chains.failure_memory_regression.repair_replay import _empirical_effect
...
def test_repeated_posterior_refits_use_the_same_observations_once(monkeypatch):
    ...
    monkeypatch.setattr("method_chains.failure_memory_regression.bayes_model.fit_logistic", fake_fit)
...
def test_task_reward_and_cross_agent_ucb_use_valid_collision():
    ...
    bank = {sid: {"scenario_id": sid, "ego_collision": True, "completed": False,
            ...
            for sid in ("a", "b")}
    queries, _observations, _cards, _updates = run_selector_v2(
    ...
    uncertain_bank = {sid: {"scenario_id": sid, "ego_collision": True,
                      ...
                      for sid in ("a", "b")}
    uncertain_queries, observations, *_ = run_selector_v2(
...
def test_regression_reward_requires_parent_pass_and_observed_fields_survive():
    ...
    queries, observations, cards, _updates = run_selector_v2(
    ...
    with pytest.raises(ValueError, match="explicit parent-pass"):
        run_selector_v2("HistoryRank-UCB-v2", [no_parent], [],
...
def test_fake_bank_replay_stays_offline_and_does_not_mutate_inputs(tmp_path):
    from method_chains.failure_memory_regression import repair_replay
    ...
    task = repair_replay._run_selector_task(
    ...
    assert "method_chains.failure_memory_regression.experiment_v2" not in sys.modules
