...
from method_chains.failure_memory_regression.archive_v2 import read_jsonl, row_to_record
from method_chains.failure_memory_regression.pattern_memory import build_pattern_cards
...
# Importing the old runner imports the simulator environment and resource registry.
# This module deliberately has no dependency on experiment_v2 or an episode executor.
...
DEFAULT_OUTPUT = PROJECT_RESULTS / "repair_20260926"
REPLAY_BUDGET = 20
RANDOM_REPEATS = 10
...
CODE_FILES = (
    "method_chains/failure_memory_regression/archive_v2.py",
    "method_chains/failure_memory_regression/bayes_model.py",
    "method_chains/failure_memory_regression/experiment_v2.py",
    "method_chains/failure_memory_regression/pattern_memory.py",
    "method_chains/failure_memory_regression/replay_utils.py",
    "method_chains/failure_memory_regression/selector_v2.py",
...
def _run_selector_task(output: Path, task_id: str, target_build: str,
                       ...
                       only_repeat: int | None = None) -> dict:
    ...
    for method in ([only_method] if only_method else METHODS):
        repeats = list(range(RANDOM_REPEATS if method == "Random" else 1))
        ...
        for repeat in repeats:
            ...
            queries, observations, learned_cards, updates = run_selector_v2(
...
def _legacy_tasks(output: Path, core: Path, fingerprint: str) -> list[dict]:
    ...
    for seed in sorted({int(row["scenario_id"].split(":", 1)[0]) for row in candidates_all}):
        ...
        for target in LEGACY_TARGETS:
            ...
            result = _cache_task(output, run_fingerprint, task_id, lambda: _run_selector_task(
                ...
                lambda method, repeat: 4179901 + repeat +
                int(stable_hash(f"{task_id}:{method}:{repeat}")[:8], 16)))
...
def _compact_regressions(output: Path, memory: Path, fingerprint: str) -> list[dict]:
...
def _make_report(report_path: Path, manifest: dict, summaries: list[dict],
    ...
    lines.extend(["", "Random entries are means across the original ten repeats; other methods use their single original run.",
                  ...
                  "- See `manifest.json`, `task_inputs.jsonl`, and `git_diff_stat.txt` for detailed provenance.", ""])
    report_path.write_text("\n".join(lines), encoding="utf-8")


def replay_bank(output: Path = DEFAULT_OUTPUT, core: Path = DEFAULT_CORE,
                memory: Path = DEFAULT_MEMORY, test_result: str = "not recorded") -> dict:
    ...
    if not core_missing:
        legacy_results = _legacy_tasks(output, core, fingerprint)
    else:
    ...
    try:
        ...
        diff = "git diff --stat unavailable"
    (output / "git_diff_stat.txt").write_text(diff, encoding="utf-8")
    return manifest
