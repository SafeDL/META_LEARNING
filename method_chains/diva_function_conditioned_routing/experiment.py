...
from diva_highway_env.diva.mining import _trace, diagnostic_mining, random_mining
...
OUTPUT_DIR = (
    ...
    / "results"
    / "method_chains"
    / "diva_function_conditioned_routing"
)
...
METHOD_COLORS = {
    ...
    "DIVA-DETOUR": "#59a14f",
    "AdaTE Global": "#f2cf5b",
    "Global Routed DIVA": "#b279a2",
    "Function Routing (risk-only)": "#ff9da6",
    "Function-Conditioned DIVA": "#d62728",
}
...
def build_benchmark_bank(config: RoutingExperimentConfig = CONFIG) -> ResponseBank:
    """Build the same six-SUT, 240-scenario bank as DIVA-DETOUR."""
...
def _diagnostic_row(
    ...
    return {
        ...
        "diva_held_out_mse": float(
            np.mean((state.diva_prediction[unseen] - truth[unseen]) ** 2)
        ),
...
def run_benchmark(
    ...
    for target_order, target_name in enumerate(bank.sut_names):
        ...
        for index, name in enumerate(("DETOUR", "DIVA", "DIVA-DETOUR")):
            traces[index] = _trace(
                name,
        ...
        routed_specs = (
            ("Global Routed DIVA", False, True, "adate"),
            ("Function Routing (risk-only)", True, False, "adate"),
            ("Function-Conditioned DIVA", True, True, "coverage"),
        )
        routed_results = []
...
def summarize(
    ...
    proposed = [
        row for row in diagnostics if row["method"] == "Function-Conditioned DIVA"
    ]
    return {
        "protocol": {
            "task": "LOSO multi-function critical-event mining",
            "alignment_reference": "method_chains/diva_detour_fusion",
        ...
        "proposed_support_prediction": {
            ...
            "mean_diva_held_out_mse_on_same_support": float(
                np.mean([float(row["diva_held_out_mse"]) for row in proposed])
            ),
...
def write_figures(
    ...
    for method in KEY_METHODS:
        axis.plot(
            ...
            linewidth=2.4 if method == "Function-Conditioned DIVA" else 1.7,
        )
    axis.set(
    ...
    axis.bar(
        x - 0.18,
        [float(row["diva_held_out_mse"]) for row in proposed],
        0.36,
        label="DIVA prediction",
        color=METHOD_COLORS["DIVA"],
    )
    axis.bar(
        x + 0.18,
        ...
        color=METHOD_COLORS["Function-Conditioned DIVA"],
    )
    axis.set(
        xticks=x,
...
def _run_and_write(
    ...
    proposed = summary["mean_metrics"]["Function-Conditioned DIVA"]
