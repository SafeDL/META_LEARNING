"""Function-aware AdaTE response routing with a DIVA fallback.
...
from diva_highway_env.diva.mining import MiningTrace, _trace
from diva_highway_env.diva.posterior import adapt_posterior
...
def routing_prediction(
    ...
    *,
    ...
    diva = np.clip(posterior.prediction, 0.0, 1.0)
    ...
    prediction = np.clip(gate_vector * routed + (1.0 - gate_vector) * diva, 0.0, 1.0)
    revealed_rmse = (
...
def sequential_diva_mining(
    ...
    """Strong baseline that updates the ordinary DIVA posterior after every query."""
