...
def encode_outcome(outcome: Mapping[str, Any], *, ttc_scale_s: float = 15.0, distance_scale_m: float = 100.0, closing_speed_scale_mps: float = 30.0) -> torch.Tensor:
    ...
    invalid = not bool(outcome.get("is_valid_episode", not any(bool(outcome.get(key, False)) for key in ("non_target_collision", "adversary_out_of_road", "sut_out_of_road", "wrong_route"))))
