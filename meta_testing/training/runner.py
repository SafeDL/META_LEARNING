...
class HierarchicalRunner:
    ...
    def rollout(self, episode: ExecutableEpisode, scene_config: Mapping[str, Any], option: str, inner_action: Callable[[Any], np.ndarray], analyze: Callable[[list[dict[str, Any]]], tuple[Mapping[str, Any], FailureSignature]], *, trajectory_extractor: TrajectoryFeatureExtractor | None = None, reward_fn: InnerRiskReward | None = None) -> Rollout:
        transitions: list[dict[str, Any]] = []
        ...
        for _ in range(self.max_steps):
            ...
            transitions.append({"state": observation, "action": action, "reward_inner": reward_fn(trajectory_row, info, option, len(transitions), self.max_steps), "reward_env": float(env_reward), "next_state": next_observation, "done": bool(terminated or truncated), "info": dict(info), "sut_observation": sut_observation, "sut_evidence": sut_evidence, "trajectory_features": trajectory_row})
