...
from ..context.set_posterior import PosteriorTrainingBatch
from ..model import HierarchicalMetaTester
from .online_meta_test import OnlineEpisode


def posterior_batch_from_episodes(model: HierarchicalMetaTester, episodes: list[OnlineEpisode]) -> PosteriorTrainingBatch:
    if len(episodes) < 2:
    ...
    def token(episode: OnlineEpisode) -> torch.Tensor:
        with torch.no_grad():
            map_embedding = model.encode_map(episode.map_tokens)[1].unsqueeze(0)
        ...
        return model.episode_token_builder(map_embedding, episode.config.to(device).unsqueeze(0), episode.option_index.to(device).reshape(1), trajectory, mask, outcome).squeeze(0)

    support, target = episodes[:-1], episodes[-1]
    support_tokens = torch.stack([token(episode) for episode in support]).unsqueeze(0)
    with torch.no_grad():
        target_map = model.encode_map(target.map_tokens)[1].unsqueeze(0)
