...
class RoutePolyline:
    ...
    def from_env(cls,
                 ...
                 samples_per_lane: int = 48) -> "RoutePolyline":
    ...
    def projection(self,
                   position: Any,
                   heading: float,
                   lane_width_m: float = 3.8) -> RouteProjection:
        point = np.asarray(position, dtype=float)
        starts, ends = self.points[:-1], self.points[1:]
        segments = ends - starts
        lengths2 = np.einsum("ij,ij->i", segments, segments)
        factors = np.clip(
            np.einsum("ij,ij->i", point - starts, segments) / np.maximum(lengths2, 1e-12), 0.0,
            1.0)
        ...
        return RouteProjection(s, lateral, tangent, wrap_to_pi(float(heading) - route_heading),
                               abs(lateral) <= 0.5 * lane_width_m)
    ...
    def tangent_at_s(self, s_m: float) -> np.ndarray:
        index = int(
            np.clip(
                np.searchsorted(self.arc_lengths_m, float(s_m), side="right") - 1, 0,
                len(self.points) - 2))
...
def route_hash_payload(route: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "lane_sequence": [list(lane_index(x)) for x in route["lane_sequence"]]
    }
