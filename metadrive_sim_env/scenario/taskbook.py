...
def validate_taskbook(tasks: Iterable[ScenarioMiningTaskSpec]) -> list[ScenarioMiningTaskSpec]:
    ...
    for task in rows:
        task.validate()
    domains_by_split = {
        split: {(task.functional_scenario, task.logical_domain_id): task
        ...
        for left_domain in domains_by_split[left].values():
            for right_domain in domains_by_split[right].values():
                ...
                for index, name in enumerate(
                    ...
                    if not (left_domain.logical_parameter_mask[index]
                            and right_domain.logical_parameter_mask[index]):
                        continue
                    left_bounds = left_domain.logical_domain_bounds[name]
                    right_bounds = right_domain.logical_domain_bounds[name]
                    if max(left_bounds[0], right_bounds[0]) <= min(left_bounds[1],
                                                                   right_bounds[1]):
                        raise ValueError(f"{left}/{right} logical domains overlap on {name}")
    hashes_by_split = {
        split: {task.geometry_hash
                for task in rows if task.geometry_split == split}
        for split in ("train", "validation", "test")
    }
    for left, right in (
        ("train", "validation"),
...
def load_taskbook(path: str | Path) -> list[ScenarioMiningTaskSpec]:
    return validate_taskbook(
        ScenarioMiningTaskSpec.from_dict(row)
