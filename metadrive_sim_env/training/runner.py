...
class HierarchicalRunner:
    def __init__(self,
                 max_steps: int = 240,
                 criteria: FailureCriteria = DEFAULT_FAILURE_CRITERIA) -> None:
    ...
    def _sut_completed_test_route(cls, episode: ExecutableEpisode) -> bool:
        ...
        completion_s = float(episode.applied_scenario.logical_parameters["test_completion_s_m"])
        projection = episode.sut_route.projection(episode.sut.position, episode.sut.heading_theta)
        return bool(
            float(projection.s_m) >= completion_s and abs(float(projection.lateral_m)) <=
    ...
    def rollout(
        ...
        *,
        ...
        controller = FrenetSACAdversaryController(episode, scenario_family, schedule)
        ...
        try:
            for step in range(max_steps):
                ...
                planner_active = bool(schedule.state.maneuver_latched
                                      and not schedule.state.maneuver_completed
                                      and reference_before.start_remaining_m <= 0.0)
                onset_just_started = bool(scenario_family == "cutin" and planner_active
                ...
                if onset_just_started:
                    ...
                    decision_seconds = (float(env.config["physics_world_step_size"]) *
                ...
                if step_action is not None:
                    ...
                    held_policy_action = np.asarray(step_action(phase),
                                                    dtype=np.float32).reshape(-1)
                elif planner_active:
                    policy_decision = (held_policy_action is None or schedule.planner.replan_due)
                else:
                    policy_decision = (held_policy_action is None
                                       or step % schedule.planner.replan_interval_steps == 0)
                if policy_decision and step_action is None:
                    held_policy_action = np.asarray(inner_action(state),
                                                    dtype=np.float32).reshape(-1)
                    if (held_policy_action.shape != (4, )
                            or not np.isfinite(held_policy_action).all()):
                        raise ValueError("Inner SAC must emit one finite 4-D planner action")
                    held_policy_action = np.clip(held_policy_action, -1.0, 1.0)
                ...
                info.update({
                    ...
                    reference.lateral_error_m,
                    "maneuver_reference_heading_error_rad":
                    reference.heading_error_rad,
                    "maneuver_reference_desired_lateral_m":
                    reference.desired_lateral_m,
                    "maneuver_reference_length_m":
                    reference.length_m,
                    "maneuver_reference_curvature_m_inv":
                    reference.curvature_m_inv,
                    "maneuver_reference_curvature_speed_limit_mps":
                    (reference.curvature_speed_limit_mps),
                    "maneuver_reference_speed_limit_mps":
                    reference.speed_limit_mps,
                    "maneuver_start_remaining_m":
                    reference.start_remaining_m,
                    "maneuver_active_lambda_length":
                    reference.active_lambda_length,
                    "maneuver_active_beta_early":
                    reference.active_beta_early,
                    "maneuver_active_beta_late":
                    reference.active_beta_late,
                    "maneuver_reference_blend_progress":
                    reference.blend_progress,
                    "maneuver_replan_due":
                    reference.replan_due,
                    "planner_active":
                    planner_active,
                    "maneuver_path_projection_scale":
                    reference.path_projection_scale,
                    "maneuver_path_speed_feasible":
                    reference.path_speed_feasible,
                    "maneuver_reference_start_s_m":
                    schedule.contract.start_s_m,
                    "maneuver_reference_points_xy":
                    schedule.planner.reference_points(),
                    "control_raw_longitudinal":
                    float(control.raw_vehicle_action[1]),
                    "control_speed_limited_longitudinal":
                    float(control.speed_limited_vehicle_action[1]),
                    "control_projected_longitudinal":
                    float(control.projected_vehicle_action[1]),
                    "control_curvature_speed_limit_mps":
                    float(control.reference.curvature_speed_limit_mps),
                    "control_distance_envelope_speed_limit_mps":
                    float(control.reference.speed_limit_mps),
                    "control_path_projection_scale":
                    float(control.reference.path_projection_scale),
                    "control_path_speed_feasible":
                    bool(control.reference.path_speed_feasible),
                    "cutin_actual_onset_just_started":
                    onset_just_started,
                    "cutin_actual_onset_time_s":
                    (None if actual_cutin_onset is None else actual_cutin_onset["time_s"]),
                    "cutin_actual_onset_gap_m":
                    (None if actual_cutin_onset is None else actual_cutin_onset["gap_m"]),
                    "cutin_actual_onset_adversary_speed_mps":
                    (None
                     if actual_cutin_onset is None else actual_cutin_onset["adversary_speed_mps"]),
                    "cutin_actual_onset_sut_speed_mps":
                    (None if actual_cutin_onset is None else actual_cutin_onset["sut_speed_mps"]),
                })
                ...
                sut_projection = episode.sut_route.projection(episode.sut.position,
                                                              episode.sut.heading_theta)
                ...
                info.update({
                    ...
                    float(
                        episode.adversary_route.projection(episode.adversary.position,
                                                           episode.adversary.heading_theta).s_m) /
                    ...
                    float(sut_action[1]),
                    "sut_lateral_error_m":
                    float(sut_projection.lateral_m),
                    "sut_heading_error_rad":
                    float(sut_projection.heading_error),
                    "sut_speed_mps":
                    float(episode.sut.speed_km_h) / 3.6,
                    "sut_route_progress_m":
                    float(sut_projection.s_m),
                    "sut_target_speed_mps":
                    float(getattr(sut_policy, "target_speed", 0.0)) / 3.6,
                    "sut_nominal_target_speed_mps":
                    float(getattr(sut_policy, "nominal_target_speed_mps", 0.0)),
                    "sut_curve_safe_speed_mps":
                    float(getattr(sut_policy, "curve_safe_speed_mps", 0.0)),
                })
                ...
                near_miss = (not target_collision and 0.0 < distance < self.criteria.distance_m
                ...
                info["inner_raw_policy_action_l2"] = float(np.square(raw_action).sum())
                info["inner_planner_action_l2"] = float(np.square(control.planner_action).sum())
                info["inner_executed_vehicle_action_l2"] = float(np.square(shielded.action).sum())
                ...
                info["requested_vehicle_action"] = (shielded.requested_action.tolist())
                info["executed_vehicle_action"] = shielded.action.tolist()
                info["valid_target_collision"] = bool(target_collision
                                                      and info["event_kind"] == "collision"
                                                      and info["event_semantic_valid"]
                                                      and info["event_execution_valid"])
                info["valid_critical_near_miss"] = bool(near_miss
                ...
                transitions.append({
                    ...
                    raw_action,
                    "planner_action":
                    control.planner_action,
                    "raw_vehicle_action":
                    control.raw_vehicle_action,
                    "speed_limited_vehicle_action": (control.speed_limited_vehicle_action),
                    "projected_vehicle_action":
                    control.projected_vehicle_action,
                    "requested_vehicle_action":
                    shielded.requested_action,
