"""Selection state for diagnostic and online Mining failure mining."""
...
class RiskMiner:
    ...
    @staticmethod
    def _select(scores: np.ndarray, available: np.ndarray, pool: tuple[CutInDesign,
                                                                       ...]) -> CutInDesign:
        if not available.any():
            raise RuntimeError("Mining candidate pool is exhausted")
    ...
    def select_diagnostic(self) -> CutInDesign:
    ...
    def select_random(self, rng: np.random.Generator) -> CutInDesign:
    ...
    def select_mining(self) -> CutInDesign:
        ...
        archive = np.asarray(self.archive_features, dtype=np.float64).reshape(
            (-1, 7)) if self.archive_features else np.empty((0, 7))
        scores = mining_scores(mean, variance, self.evaluability,
                               novelty_weight(features, archive))
        design = self._select(scores, self._available(), self.pool)
        self.selected_ids.add(design.design_id)
        return design

    def select_source_mean(self) -> CutInDesign:
    ...
    def observe(self, observation: MiningObservation) -> None:
        if observation.design.design_id not in self.selected_ids:
            raise ValueError("Mining observation was not selected by this miner")
        if observation.posterior_eligible:
            ...
            self.posterior.update(basis[0], observation.vulnerability_response - shared_mean,
                                  noise[0])
