...
from ..mining.behavior import CutInBehavior
...
def _load_observations(path: Path) -> list[dict[str, Any]]:
    ...
    if not rows or any(row.get("schema") != MINING_SCHEMA for row in rows):
...
def _plot_summary(
    ...
    figure.suptitle("Risk Mining Stage A | Cut-in source evidence", fontsize=18, fontweight="bold")
    ...
    for row in rows:
        by_design.setdefault(row["design"]["design_id"],
                             {})[row["sut_ref"]] = row["vulnerability_response"]
    designs = sorted(by_design,
                     key=lambda design: np.mean(list(by_design[design].values())),
                     reverse=True)
    response_matrix = np.asarray([[by_design[design][source] for design in designs]
                                  for source in SOURCE_ORDER])
    axis = axes[1, 0]
    ...
    methods = (
        ...
        ("diagnostic_support_k", "Mining diagnostic", "#E45756", "D"),
    )
    for prefix, label, color, marker in methods:
        if prefix == "shared_prior_k0":
            axis.scatter([0], [aggregates[prefix]["ndcg_at_8"]],
                         s=90,
                         color=color,
                         marker=marker,
                         label=label,
...
def _draw_reference_overlay(
    ...
    *,
    ...
    if view == "topdown":
        ...
        for point in points:
            relative = camera.getRelativePoint(episode.env.engine.render,
...
def _annotated_frame(
    ...
    dt = float(episode.env.config["physics_world_step_size"]) * int(
        episode.env.config["decision_repeat"])
    height = min(chase.shape[0], topdown.shape[0])
    chase_image = Image.fromarray(chase).resize(
        (round(chase.shape[1] * height / chase.shape[0]), height))
    topdown_image = Image.fromarray(topdown).resize(
        (round(topdown.shape[1] * height / topdown.shape[0]), height))
    action = info.get("raw_policy_action", (0.0, 0.0, 0.0, 0.0))
    left = _panel(
        np.asarray(chase_image),
        "Tail view | blue: SUT | red: Mining adversary",
...
def _render_replay(rows: list[Mapping[str, Any]], config: Mapping[str, Any],
                   output: Path) -> dict[str, Any]:
    candidates = [
        row for row in rows if row["sut_ref"] == "idm_assertive" and float(row["score"]) == 1.0
    ]
    ...
    base_task = retained_task(load_taskbook(config["taskbook"]), row["sut_ref"],
                              config["study"]["task_logical_domain_id"])
    task = study_task(base_task, row["logical_domain_id"],
    ...
    episode = ScenarioExecutor(load_adapters(), mvr_parameter_spaces()).reset(
        ...
        environment_overrides=VISUAL_OVERRIDES
        | {"horizon": int(config["execution"]["environment_horizon"])},
    )
    ...
    try:
        rollout = HierarchicalRunner(int(config["execution"]["runner_step_budget"])).rollout(
            episode, "cutin", step_action=CutInBehavior(), step_callback=capture)
    finally:
        episode.env.close()
...
def run(config_path: str, observations_path: str, loso_path: str,
    ...
    report = {
        "schema": "mining_stage_a_visualization_v2",
