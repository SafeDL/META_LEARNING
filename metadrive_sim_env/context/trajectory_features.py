...
class TrajectoryFeatureExtractor:
    ...
    scales = np.asarray(
        (100.0, 100.0, 30.0, 10.0, 30.0, 10.0, 200.0, 200.0, 15.0, 15.0, 100.0, 15.0),
        dtype=np.float32)
    ...
    def reset(
        ...
        self._adversary_route = adversary_route or RoutePolyline.from_env(
            env, {
                "route_id": "adversary",
                "lane_sequence": list(layout.adversary_route)
            })
        self._sut_route = sut_route or RoutePolyline.from_env(
            env, {
                "route_id": "sut",
                "lane_sequence": list(layout.sut_route)
            })
    ...
    def step(self, env: Any, adversary: Any, sut: Any, info: Any) -> np.ndarray:
        ...
        adv_position, sut_position = np.asarray(adversary.position,
                                                dtype=float), np.asarray(sut.position, dtype=float)
        ...
        adv_velocity, sut_velocity = np.asarray(adversary.velocity,
                                                dtype=float), np.asarray(sut.velocity, dtype=float)
        ...
        acceleration = 0.0 if self._previous_sut_speed is None else (
            sut_speed - self._previous_sut_speed) / max(dt, 1e-6)
        ...
        values = np.asarray(
            (rel_local[0], rel_local[1], sut_speed - adv_speed, acceleration, sut_speed,
             sut_projection.lateral_m, adv_projection.s_m, sut_projection.s_m, ttc,
             abs(adv_eta - sut_eta), distance, adv_eta - sut_eta),
            dtype=np.float32)
        normalized = np.clip(np.nan_to_num(values / self.scales, nan=0.0, posinf=1.0, neginf=-1.0),
                             -1.0, 1.0)
