...
from metadrive_sim_env.scenario.catalog import mvr_parameter_spaces
from metadrive_sim_env.scenario.executor import ScenarioExecutor
from metadrive_sim_env.scenario.parameter_space import NormalizedScenarioAction
from metadrive_sim_env.scenario.semantics import ScenarioSemanticMonitor
from metadrive_sim_env.scenario.taskbook import load_taskbook
from metadrive_sim_env.scenario.registry import load_adapters
from metadrive_sim_env.training.runner import HierarchicalRunner
from sut_algorithms.metadrive.idm import LaneStableNativeIDMPolicy
...
def test_initial_speed_bounds_follow_family_traffic_contract() -> None:
    ...
    assert spaces["cutin"].bounds["ego_initial_speed_mps"] == (7.0, 13.0)
    assert spaces["cutin"].bounds["relative_speed_mps"] == (-2.5, 2.5)
...
def test_idm_sut_stays_on_its_declared_route_centerline(family: str) -> None:
    ...
    def record_sut_tracking(current, _step, info) -> None:
        projection = current.sut_route.projection(current.sut.position, current.sut.heading_theta)
    ...
    try:
        ...
        assert all(row["sut_current_lane"][2] == row["sut_expected_lane_number"]
...
def test_runtime_hash_mismatch_fails_fast() -> None:
    task = load_taskbook("metadrive_sim_env/configs/taskbook.json")[0]
    broken = type(task)(**{**task.to_dict(), "geometry_hash": "0" * 64})
    with pytest.raises(RuntimeError, match="map hash mismatch"):
        ScenarioExecutor(load_adapters(),
                         mvr_parameter_spaces()).reset(broken,
...
def test_merge_always_assigns_branch_entry_to_the_adversary(candidate_index: int) -> None:
    ...
    try:
        ...
        adversary_source_width = len(
            network.graph[layout.adversary_lane[0]][layout.adversary_lane[1]])
...
@pytest.mark.parametrize("candidate_index", (0, 1))
def test_merge_adversary_physically_enters_the_sut_downstream_lane(candidate_index: int, ) -> None:
    task = next(row for row in load_taskbook("metadrive_sim_env/configs/taskbook.json")
                if row.task_id == "merge-g04-fast_small_gap-interaction_core")
    episode = ScenarioExecutor(load_adapters(), mvr_parameter_spaces()).reset(
        task,
    ...
    def record_merge_runtime(current, _step, info) -> None:
        ...
        overlaps = (ScenarioSemanticMonitor._vehicle_overlaps_lane_corridor(
...
def test_roundabout_entry_probe_preserves_physical_route_tracking() -> None:
    ...
    task = replace(
        ...
        logical_domain_bounds={name: (-1.0, 1.0)
                               for name in task.logical_domain_bounds},
    )
    ...
    episode = ScenarioExecutor(load_adapters(),
                               mvr_parameter_spaces()).reset(task,
                                                             action,
                                                             episode_seed=task.geometry_seed + 1)
    try:
