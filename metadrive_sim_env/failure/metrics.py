...
class FixedBudgetMetrics:
    ...
    def summary(self) -> dict[str, float | int | None]:
        ...
        first = next((index + 1 for index, item in enumerate(self.signatures) if item.is_failure),
                     None)
        target = lambda count: next(
            (index + 1 for index, value in enumerate(curve) if value >= count), None)
        return {
            "episodes":
            len(self.signatures),
            "budget":
            ...
            curve[-1] if curve else 0,
            "failure_discovery_auc":
            float(np.trapezoid(curve, dx=1.0)) if curve else 0.0,
            ...
            target(10),
            "invalid_rate":
            float(1.0 - sum(item.is_valid_episode
                            for item in self.signatures) / len(self.signatures))
            if self.signatures else 0.0,
            "unique_failures_per_episode":
            float((curve[-1] if curve else 0) / max(1, len(self.signatures))),
        }
