...
class VehicleActionProjector:
    ...
    def _steering_limit(self) -> float:
        speed = max(
            self._speed_mps(self.vehicle) +
            self.max_acceleration_mps2 * self._step_seconds(self.vehicle),
    ...
    def project(self, requested_action: Any) -> np.ndarray:
        ...
        action[0] = np.clip(action[0], current_steering - steering_delta,
                            current_steering + steering_delta)
        action[0] = np.clip(action[0], -self._steering_limit(), self._steering_limit())
        requested_acceleration = float(action[1]) * (self.max_acceleration_mps2 if action[1] >= 0.0
                                                     else self.max_deceleration_mps2)
        ...
        action[1] = acceleration / (self.max_acceleration_mps2
                                    if acceleration >= 0.0 else self.max_deceleration_mps2)
