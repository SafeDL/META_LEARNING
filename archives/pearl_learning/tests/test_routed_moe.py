...
from archives.pearl_learning.src.checkpoint import CHECKPOINT_SCHEMA, METHOD_CONTRACT, load_checkpoint, save_checkpoint
from archives.pearl_learning.src.collector import collect_episode
from archives.pearl_learning.src.io import read_config
from archives.pearl_learning.src.moe import (
    DESCRIPTOR_FIELDS,
    DESCRIPTOR_SCHEMA,
...
from archives.pearl_learning.src.replay import Transition


def small_config(*, architecture: str = "posterior_routed_moe", routing: str = "soft",
                 top_k: int = 2) -> dict:
    config = read_config("archives/pearl_learning/configs/posterior_routed_moe.yaml")
