"""Offline re-judgement of Gate 3 audit suites under the v4 causal contract.

The source suites already contain posterior, Critic-grid, Actor, and query
metrics.  This tool therefore consumes no environment steps and writes an
additive ``gate3_causal_chain_gate_v4.json`` beside each suite, preserving all
...
from archives.pearl_learning.scripts.audit_gate3_vanilla_pearl_mechanism import gate3_causal_chain_verdict_v4
...
def recompute(suite_path: Path, oracle: dict[str, Any] | None, output_dir: Path | None) -> dict[str, Any]:
    ...
    target = (output_dir if output_dir is not None else suite_path.parent) / "gate3_causal_chain_gate_v4.json"
...
def main() -> None:
    ...
    for raw in args.suite:
        ...
        print(
            ...
            f"Stage B_Q {stages['stage_b_q_posterior_to_critic_action_preference']['status']}, "
            f"Stage B_pi {stages['stage_b_pi_critic_to_actor_action']['status']}, "
