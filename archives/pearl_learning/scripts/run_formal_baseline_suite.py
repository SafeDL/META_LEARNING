...
from archives.pearl_learning.src.baselines import PRETRAIN_BASELINE_NAMES
...
def main() -> None:
    ...
    for baseline, command in commands:
        if _is_complete(root, baseline, taskbook_hash, args.environment_steps):
            ...
            subprocess.run([sys.executable, "-m", "archives.pearl_learning.scripts.run_baselines", *command], check=True)
        ...
        if baseline == "per_task_sac":
            subprocess.run([
                sys.executable, "-m", "archives.pearl_learning.scripts.select_per_task_sac_checkpoints",
                "--config", args.config,
        ...
        if baseline == "topology_conditioned_pooled_sac":
            subprocess.run([
                sys.executable, "-m", "archives.pearl_learning.scripts.select_pooled_sac_checkpoint",
                "--config", args.config,
                "--taskbook", args.taskbook,
                "--casebook-root", args.casebook_root,
                "--baseline-root", args.output,
