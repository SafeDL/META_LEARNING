...
python -m archives.sac_scenario_mining.scripts.train_sac --seed 0 --run-name merge_sac_seed0
python -m archives.sac_scenario_mining.scripts.evaluate --policy random --split test --seed 123
# 默认从固定 held-out case 表抽取 5 个互不重复的场景
python -m archives.sac_scenario_mining.scripts.visualize --selection-seed 42
# 如需锁定单个场景：
python -m archives.sac_scenario_mining.scripts.visualize --case-id test_000
python -m archives.sac_scenario_mining.scripts.replay --manifest results/sac_scenario_mining/final_eval/sac_seed2/critical_scenarios/rank_001/manifest.json
```
