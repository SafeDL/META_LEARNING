...
from method_chains.failure_memory_regression.bayes_model_full_cov_v3 import (
...
METHODS = ("Random", "HistoryRank-UCB-v2", "FailureDistance-v2",
           "FBRT-NoMemory", "FBRT-Memory", "FBRT-Memory-Weighted-v3",
           "FBRT-Memory-FullCov-v3")
MEMORY_METHODS = {"FBRT-Memory", "FBRT-Memory-Weighted-v3",
                  "FBRT-Memory-FullCov-v3"}
...
def run_selector_v2(method: str, candidates: list[dict], history: list[dict],
                    ...
                    initial_cards=None,
                    initial_history_weight: float = 0.5
    ...
    if not 0.0 <= initial_history_weight <= 1.0:
        raise ValueError("initial history weight must be in [0, 1]")
    ...
    for template, dictionary in dictionaries.items():
        ...
        history_weights[template] = (initial_history_weight
                                     if method in MEMORY_METHODS and source_ids[template]
                                     else 0.0)
        ...
        if method == "Random":
            ...
            if rank in (10, 20):
                ...
                for template_name in {item["template_id"] for item in pool}:
                    ...
                    if (history_weights[template_name] > 0 or
                            (method in {"FBRT-Memory-Weighted-v3", "FBRT-Memory-FullCov-v3"} and
                             source_ids[template_name])):
